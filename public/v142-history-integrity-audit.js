(function installV142HistoryAudit(global){
  ...
  async function load(){
    ...
    body.innerHTML='<span class="status-pill warning">正在只读核对历史日报、快照和7业务导出覆盖…</span><p class="muted">V451 先读取每日有效 snapshotId，再按 snapshotId 索引读取明细；不会执行会锁住27GB SQLite 的轨迹/扫描/OPEN大表统计。</p>';
    ...
    try{
      ...
      if(diagnosticsSkipped){
        evidenceHtml=`<p class="muted"><b>安全读取模式：</b>已跳过 OPEN/扫描/轨迹的大表统计，避免它们阻塞主SQLite；这些项目是“未执行统计”，不是0票，也不会被拿来放宽导出安全判断。</p><p class="muted">当前安全判定仍严格依据：每日有效导入、核心快照/最终明细覆盖、WHPP日报与最终明细、CEAF来源归属。任一必要读取失败或必要数据不完整都会阻止导出。</p>`;
      }else{
  ...
  global.__CE_QC_V142_HISTORY_AUDIT__={version:VERSION,v451UiId:V451_UI_ID,refresh:load,get running(){return auditRunning;},get lastLoadedAt(){return lastLoadedAt;},automatic:false};
  console.info('[CE-QC][V142_HISTORY_AUDIT]',VERSION,V451_UI_ID,'manual audit uses snapshot-indexed readonly reads and never mutates primary business data.');
})(window);
