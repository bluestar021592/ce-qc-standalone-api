(function installUnifiedClosureRateV133(global){
  ...
  const VERSION='2026-08-14-v133-unified-closure-rate-v1';
  ...
  function patchWhppFromDom(){
    ...
    const closed=Math.max(0,total-unresolved);
    const summary=cached?.byBusiness?.WHPP;
    const completed=summary?.completed!==false && !/尚未完成独立扫描\/轨迹/.test(String(page.querySelector('.v18-page-heading p')?.textContent||''));
  ...
  function scheduleWhppPatch(){[0,80,300,900].forEach(ms=>setTimeout(patchWhppFromDom,ms));}
  function install(){
    wrapDashboards();
