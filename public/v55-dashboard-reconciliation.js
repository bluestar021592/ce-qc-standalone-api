(function installDashboardReconciliationV55(global){
  const VERSION='2026-08-11-v55-dashboard-reconciliation-ui-v3';
  ...
  async function authoritativeSummary(type=exactType()){
    ...
    if(local&&Number.isFinite(Number(local.total)))return local;
    const r=range();if(!r.to)return null;
    const key=`${type}|${r.from}|${r.to}`;
    if(summaryCache.has(key))return summaryCache.get(key);
    try{
      const q=new URLSearchParams({from:r.from,to:r.to});
      const response=await fetch(`/api/v55/reconciliation?${q}`,{cache:'no-store',credentials:'same-origin'});
      const data=await response.json().catch(()=>({}));
      if(!response.ok||data.ok===false)throw new Error(data.error||`HTTP ${response.status}`);
      const value=data.summary?.[type]||null;
      if(value)summaryCache.set(key,value);
      return value;
    }catch(error){console.warn('[CE-QC][V55][SUMMARY]',error);return null;}
  }

  function ccslValues(s){return{
    '签收件数':s.pod,'今日POD':s.pod,'已退回件':s.returned,'当前未闭环':s.open,'未闭环':s.open,
    '签收率':s.podRate,'POD率':s.podRate,'对账差异':s.accountingDifference,
    ...
    '盘点2天+':s.cycle2,'盘点 2天+':s.cycle2,'入库无扫描':s.inboundNoScan,'入库无扫描节点':s.inboundNoScan,
    '工单':s.workOrder,'工单未处理':s.workOrder,'严重异常':s.abnormal,'外省未完结POD件':s.provinceOpen,
    'CCSLCN分流':s.ccslCnDiversion,'CCSLZT分流':s.ccslZtDiversion,'580滞留包裹':s.ccsl580Retention,
    '金边门店':s.phnomPenhShop,'外省门店':s.provinceShop
  };}
  function shopeeValues(s){return{
    '今日POD':s.pod,'签收件数':s.pod,'POD率':s.podRate,'签收率':s.podRate,'已退回件':s.returned,'退件率':s.returnRate,'退回率':s.returnRate,
    '当前未闭环':s.unresolved,'未闭环':s.unresolved,'Pending不连续':s.pendingNonContinuous,'Pending1+':s.pending1,'Pending2+':s.pending2,'Pending3+':s.pending3plus,
    'OC1+':s.oc1,'OC2+':s.oc2,'OC3+':s.oc3plus,'盘点2天+':s.cycle2plus,'入库无扫描':s.inboundNoScan,'派送中':s.deliveryStay
  };}
  ...
  function patchModel(model,s){
    if(!model||!s)return model;
    const type=String(model.businessType||'').toUpperCase();
    const total=Number(s.total||0),values=String(type).startsWith('SHOPEE')?shopeeValues(s):ccslValues(s);
    for(const row of [...(model.cards||[]),...(model.core||[])]){
      ...
      if(Object.hasOwn(values,label)){row.value=Number(values[label]||0);row.ratio=isRateLabel(label)?`当前比率 ${Number(row.value||0).toFixed(2)}%`:pct(row.value,total);}
    }
    if(!String(type).startsWith('SHOPEE')){
      const wanted=['未闭环','严重异常','CCSLCN分流','CCSLZT分流','580滞留包裹','金边门店','外省门店'];
      const cleaned=[];const seen=new Set();
      for(const row of model.core||[]){
        const label=canonicalLabel(row.label);if(['门店途中','门店入库','门店滞留'].includes(label))continue;
        if(wanted.includes(label))continue;row.label=label;const key=`${label}|${row.unit||'件'}`;if(!seen.has(key)){seen.add(key);cleaned.push(row);}
      }
      for(const label of wanted)cleaned.push({key:`v55-${label}`,label,value:Number(values[label]||0),unit:'件',ratio:pct(values[label],total)});
      model.core=cleaned;
    }
    return model;
  }
  ...
  function installRenderWrapper(){
    ...
    global.DashboardV18.renderBusiness=function(root,model){
      const s=localSummary();if(s)patchModel(model,s);
      const result=original.call(this,root,model);
      scheduleSync();return result;
    };
    global.DashboardV18.__v55Reconciled=true;return true;
  }
  ...
  function setCard(card,value,label,total){
    if(value===undefined)return;
    const number=Number(value||0),valueNode=card.querySelector('b'),note=card.querySelector('em,small');
    if(valueNode)valueNode.textContent=isRateLabel(label)?`${number.toFixed(2).replace(/\.00$/,'')}%`:fmt(number);
    if(note)note.textContent=isRateLabel(label)?`当前比率 ${number.toFixed(2)}%`:pct(number,total);
  }
  function ensureCoreCard(grid,type,label,value,total){
    if([...grid.querySelectorAll('.v18-metric-card')].some(card=>cardLabel(card)===label))return;
    const button=document.createElement('button');button.className='v18-metric-card';button.dataset.metric=`v55-${label}`;
    button.innerHTML=`<i aria-hidden="true">●</i><span>${esc(label)}</span><b>${fmt(value)}</b><small>${esc(pct(value,total))}</small>`;
    button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();const tab=tabFor(type,label);if(tab)void openDetail(type,tab,label,1);});grid.appendChild(button);
  }
  async function syncVisibleBusinessPage(){
    if(syncing)return;const type=exactType();if(!type)return;
    syncing=true;
    try{
      const s=await authoritativeSummary(type);if(!s)return;const total=Number(s.total||0);
      document.querySelectorAll('.v18-business-grid .v18-business-card,.v18-core-grid .v18-metric-card').forEach(card=>{
        const label=cardLabel(card);
        if(label===type||label===type.replace('SHOPEE','SHOPEE '))setCard(card,total,label,total);
        else setCard(card,valueFor(type,s,label),label,total);
      });
      if(!String(type).startsWith('SHOPEE')){
        const grid=document.querySelector('.v18-core-grid');if(grid){
          for(const label of ['未闭环','严重异常','CCSLCN分流','CCSLZT分流','580滞留包裹','金边门店','外省门店'])ensureCoreCard(grid,type,label,Number(valueFor(type,s,label)||0),total);
        }
      }
      document.documentElement.dataset.v55Reconciled='1';
    }finally{syncing=false;}
  }
  function scheduleSync(){clearTimeout(syncTimer);syncTimer=setTimeout(()=>void syncVisibleBusinessPage(),25);}
  ...
  function tabFor(type,label,key=''){
    ...
    const common={
      ...
      'OC1+':'ocAll','OC 1天+':'ocAll','OC2+':'oc2plus','OC 2天+':'oc2plus','OC3+':'oc3','OC 3天+':'oc3','盘点2天+':'cycle2','盘点 2天+':'cycle2','入库无扫描':'inboundNoScan','入库无扫描节点':'inboundNoScan','工单':'workOrderAbnormal','工单未处理':'workOrderAbnormal',
    ...
    const sh={'今日POD':'pod','POD率':'pod','签收件数':'pod','签收率':'pod','已退回件':'returned','退件率':'returned','退回率':'returned','当前未闭环':'unresolved','未闭环':'unresolved','Pending不连续':'pendingNonContinuous','Pending1+':'pending1','Pending2+':'pending2','Pending3+':'pending3','OC1+':'oc1','OC2+':'oc2','OC3+':'oc3','盘点2天+':'cycle2','入库无扫描':'inboundNoScan','派送中':'deliveryStay','退回待处理':'returnRequired'};
  ...
  async function openDetail(type,tab,label,pageNo=1){
    const host=panel(type),r=range();if(!host||!r.to)return false;
    ...
    try{
      ...
      if(!response.ok||data.ok===false)throw new Error(data.error||`HTTP ${response.status}`);renderDetail(host,data,label);return true;
    }catch(error){host.innerHTML=`<div class="empty-state">明细读取失败：${esc(error.message||error)}</div>`;return false;}
  }

  function installDirectDrilldown(){
    if(global.__CE_QC_V55_DIRECT_DRILLDOWN__)return;global.__CE_QC_V55_DIRECT_DRILLDOWN__=true;
    const previousV18=global.openV18MetricDetail;
    global.openV18MetricDetail=function v55OpenV18MetricDetail(type,key,label){
      const exact=exactType()||String(type||'').toUpperCase();const tab=tabFor(exact,label,key);
      if(exact&&tab){void openDetail(exact,tab,canonicalLabel(label),1);return false;}
      if(typeof previousV18==='function')return previousV18.apply(this,arguments);
      return false;
    };
    const previousMetric=global.openMetricDetail;
    global.openMetricDetail=function v55OpenMetricDetail(type,metricKey,label,extra={}){
      const exact=exactType()||String(type||'').toUpperCase();const tab=tabFor(exact,label,metricKey);
      if(exact&&tab){void openDetail(exact,tab,canonicalLabel(label),1);return false;}
      if(typeof previousMetric==='function')return previousMetric.apply(this,arguments);
      return false;
    };
    global.addEventListener('click',event=>{
      const type=exactType();if(!type)return;const card=event.target?.closest?.('.v18-metric-card,.v18-business-card');if(!card)return;
      const label=cardLabel(card),tab=tabFor(type,label,card.dataset.metric||'');if(!tab)return;
  ...
  function installObserver(){
    ...
    const observer=new MutationObserver(()=>{if(exactType())scheduleSync();});observer.observe(document.documentElement,{subtree:true,childList:true});
  }
  function install(){
    installDirectDrilldown();installObserver();
    if(!installRenderWrapper()){let attempts=0;const timer=setInterval(()=>{attempts++;if(installRenderWrapper()||attempts>40){clearInterval(timer);scheduleSync();}},50);}else scheduleSync();
    global.addEventListener('popstate',scheduleSync);global.addEventListener('hashchange',scheduleSync);
