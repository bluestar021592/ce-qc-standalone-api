(function installDashboardReconciliationV55(global){
  const VERSION='2026-08-11-v55-dashboard-reconciliation-ui-v5';
  ...
  function canonicalLabel(label=''){
    ...
    if(['CCSLCN','CCSLCN分流','CECN滞留包裹','CCSLCN滞留包裹'].includes(value))return'CCSLCN';
    if(['CEZT','CCSLZT分流','CEZT滞留包裹','CCSLZT滞留包裹'].includes(value))return'CEZT';
    if(['CCSL580','580滞留包裹','CCSL580分流','CCSL580滞留包裹'].includes(value))return'CCSL580';
    return value;
  }
  ...
  function normalizeCardLabel(card){const node=card?.querySelector('.v18-metric-label')||card?.querySelector('span');if(!node)return'';const label=canonicalLabel(node.textContent||'');if(label&&node.textContent.trim()!==label)node.textContent=label;const metric=String(card.dataset?.metric||'');if(metric.startsWith('v55-'))card.dataset.metric=`v55-${canonicalLabel(metric.slice(4))}`;return label;}
  function dedupeCoreCards(grid){if(!grid)return;const seen=new Set();for(const card of [...grid.querySelectorAll('.v18-metric-card')]){const label=normalizeCardLabel(card)||cardLabel(card);if(!label)continue;if(seen.has(label)){card.remove();continue;}seen.add(label);}}
  function ensureCoreCard(grid,type,label,value,total){label=canonicalLabel(label);dedupeCoreCards(grid);if([...grid.querySelectorAll('.v18-metric-card')].some(card=>cardLabel(card)===label))return;const button=document.createElement('button');button.className='v18-metric-card';button.dataset.metric=`v55-${label}`;button.innerHTML=`<i aria-hidden="true">●</i><span>${esc(label)}</span><b>${fmt(value)}</b><small>${esc(pct(value,total))}</small>`;button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();const tab=tabFor(type,label);if(tab)void openDetail(type,tab,label,1);});grid.appendChild(button);}
  async function syncVisibleBusinessPage(){
    if(syncing)return;const type=exactType();if(!type)return;syncing=true;
    try{const s=await authoritativeSummary(type);if(!s)return;const total=Number(s.total||0);const grid=document.querySelector('.v18-core-grid');dedupeCoreCards(grid);document.querySelectorAll('.v18-business-grid .v18-business-card,.v18-core-grid .v18-metric-card').forEach(card=>{const label=normalizeCardLabel(card)||cardLabel(card);if(label===type||label===type.replace('SHOPEE','SHOPEE '))setCard(card,total,label,total);else setCard(card,valueFor(type,s,label),label,total);});if(!String(type).startsWith('SHOPEE')&&grid){for(const label of ['未闭环','严重异常','CCSLCN','CEZT','CCSL580','金边门店','外省门店'])ensureCoreCard(grid,type,label,Number(valueFor(type,s,label)||0),total);dedupeCoreCards(grid);}document.documentElement.dataset.v55Reconciled='2';}finally{syncing=false;}
  }
