(function installRangeTrendsV137(global){
  ...
  const VERSION='2026-08-15-v141-range-trends-data-availability-v3';
  ...
  function hideLegacy(root){
    if(!root)return;
    root.querySelectorAll('.v18-trend-section:not(.v137-exclusive-trends)').forEach(node=>{node.hidden=true;node.style.display='none';node.dataset.replacedBy='V141';});
    root.querySelectorAll('#v27ForcedHomeAttemptTrends,#v27ForcedAttemptTrend,.v27-force-trend,.v27-force-attempt,.v27-attempt-section').forEach(node=>{node.hidden=true;node.style.display='none';node.dataset.replacedBy='V141';});
  ...
  function applySelectedDateAvailability(data,businessType){
    ...
    banner.innerHTML=`<b>${r.to} 没有 VALID + COMPLETED 的有效日报快照。</b><small>因此本日核心指标不能按“0”解释，系统已隐藏这批0值卡片。${latest?`下方趋势仅展示最近有效日报，最新到 ${latest}。`:''}</small>`;
  ...
  const previousRenderAll=global.renderAll;if(typeof previousRenderAll==='function')global.renderAll=function v141RenderAll(...args){const result=previousRenderAll.apply(this,args);schedule(30);return result;};
  ...
  console.info('[CE-QC][V141_RANGE_TRENDS]',VERSION);
})(window);
