...
async function rawHealthProbe(timeoutMs = 3500) {
...
async function api(url, options = {}) {
  ...
  try {
    ...
    const error = new Error(backendHealthy
      ? '当前接口请求中断，但后台服务仍在线，请稍后重试'
      : '与后台的连接已中断，请稍候，系统将自动恢复任务状态');
    error.code = backendHealthy ? 'API_REQUEST_INTERRUPTED' : 'NETWORK_CONNECTION_INTERRUPTED';
    error.backendHealthy = backendHealthy;
    error.cause = cause;
    throw error;
  }
  const rawText = await response.text();
