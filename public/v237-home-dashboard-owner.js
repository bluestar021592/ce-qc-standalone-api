(function installV237HomeDashboardOwner(global){
  ...
  const VERSION='2026-08-22-v237-home-dashboard-owner-v2';
  ...
  function setBusinessCards(root,payload){
    ...
    const entries=Object.entries(byType);const grand=entries.reduce((sum,[,row])=>sum+num(row?.total),0);
  ...
  const observer=new MutationObserver(records=>{if(!onHome())return;const relevant=records.some(record=>[...record.addedNodes].some(node=>node?.nodeType===1&&(node.matches?.('#homePage,.v18-business-grid,.v18-mid-grid,.v18-trend-section')||node.querySelector?.('.v18-business-grid,.v18-mid-grid,.v18-trend-section'))));if(relevant)schedule(80,false);});
  ...
  console.info('[CE-QC][V237_HOME_DASHBOARD_OWNER]',VERSION,'homepage business cards + SHOPEE specialty + four trends use V237 exact daily truth');
})(window);
