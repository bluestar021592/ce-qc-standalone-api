(function installV237HomeDashboardOwner(global){
  ...
  const VERSION='2026-08-23-v247-home-ledger-truth-owner-v1';
  ...
  function removeDuplicateHomeAssessment(root){
    const duplicate=new Set(['首次妥投率','首日POD妥投率','首日妥投率','首日POD']);
    root.querySelectorAll('.v18-core-grid .v18-metric-card').forEach(card=>{const label=String(card.querySelector('span')?.textContent||'').trim();if(duplicate.has(label))card.remove();});
  }
  function applyHomeCurrent(payload){
    ...
    removeDuplicateHomeAssessment(root);
  }
  ...
  function renderTrends(allTrend){
    ...
    const specs=[
      ...
      {title:'POD数量趋势',type:'count',dates,series:[series('POD数量','#16a36a',values(allTrend,'pod'))]},
      {title:'POD率趋势',type:'rate',dates,series:[series('POD率','#6c4cf5',values(allTrend,'podRate'))]},
      {title:'OC数量趋势',type:'count',dates,series:[series('当日OC','#ff8a00',values(allTrend,'ocCurrent'))]}
    ];
  ...
  function findHomeAttemptSection(root){
    for(const h of root.querySelectorAll('h2,h3,h4')){const t=String(h.textContent||'');if(/SHOPEE\s*1\s*\/\s*2\s*\/\s*3派|1\s*\/\s*2\s*\/\s*3派成功率趋势/.test(t))return h.closest('section')||h.closest('article');}
    return root.querySelector('#v247HomeShopeeAttempts');
  }
  function attemptSummary(data){
    const rows=Array.isArray(data?.daily)?data.daily:[];const pod=rows.reduce((s,r)=>s+num(r.pod),0),unknown=rows.reduce((s,r)=>s+num(r.attemptUnknown),0),known=Math.max(0,pod-unknown),ready=rows.filter(r=>r.ledgerReady).length;
    return{pod,unknown,known,ready,totalDays:rows.length};
  }
  function renderAttemptChart(card,data,label){
    if(!card||!global.RateTrendCardV18?.render)return;
    global.RateTrendCardV18.render(card,{title:`${label} 1/2/3派签收占POD趋势`,type:'rate',dates:data?.dates||[],series:[
  ...
  function renderHomeAttempts(cn,vn){
    const root=homeRoot();if(!root)return;let host=findHomeAttemptSection(root);
    ...
    host.id='v247HomeShopeeAttempts';host.className='v18-panel';host.dataset.v247=VERSION;
    const c=attemptSummary(cn),v=attemptSummary(vn);
    const note=(name,s)=>s.totalDays===0?`${name}：暂无有效日报`:(s.ready<s.totalDays?`${name}：V246账本正在补齐（${s.ready}/${s.totalDays}个日报日已锁定）；不显示部分数据假0%`:`${name}：已识别派次 ${s.known}/${s.pod} POD，未识别 ${s.unknown}票`);
    host.innerHTML=`<div style="padding:14px 16px 6px"><h2 style="margin:0;color:#17365d">SHOPEE 1/2/3派签收占POD趋势</h2><p style="margin:6px 0 0;color:#7a6a3a;font-size:12px;line-height:1.7">${note('CN',c)}<br>${note('VN',v)}。派次严格按真实70 START → Pending/失败 → 新START计算；无证据保持“—”。</p></div><div class="v18-chart-grid"><article class="v18-chart-card" data-v247-attempt="CN"></article><article class="v18-chart-card" data-v247-attempt="VN"></article></div>`;
    renderAttemptChart(host.querySelector('[data-v247-attempt="CN"]'),cn,'SHOPEE CN');
    renderAttemptChart(host.querySelector('[data-v247-attempt="VN"]'),vn,'SHOPEE VN');
  }
  function latestDaily(data){const rows=Array.isArray(data?.daily)?data.daily:[];return rows.at(-1)||null;}
  function applyHomeDispatchTruth(cn,vn){
    const root=homeRoot();if(!root)return;const cnDay=latestDaily(cn),vnDay=latestDaily(vn);
    const byLabel={'CN-PP':cnDay?.regions?.PP,'CN-PV':cnDay?.regions?.PV,'VN-PP':vnDay?.regions?.PP,'VN-PV':vnDay?.regions?.PV};
    root.querySelectorAll('.v18-dispatch-grid > div').forEach(node=>setDispatchGroup(node,byLabel[String(node.querySelector('h3')?.textContent||'').trim()]||null));
  }
  ...
  function scheduleLedgerRetry(cn,vn){
    clearTimeout(ledgerRetryTimer);if(!ledgerNeedsRetry(cn)&&!ledgerNeedsRetry(vn)){ledgerRetryCount=0;return;}if(ledgerRetryCount>=24)return;ledgerRetryCount+=1;
    ledgerRetryTimer=setTimeout(()=>{if(!onHome())return;const r=range();for(const t of ['SHOPEECN','SHOPEEVN'])cache.delete(`v247|${t}|${r.from}|${r.to}`);void refreshHome(false);},15000);
  }
  async function refreshHome(force=false){
    if(!onHome()||!homeRoot())return;const r=range();if(!r.to)return;const id=++requestId;
    try{
      const [current,allTrend,cnTrend,vnTrend]=await Promise.all([
        ...
        shopeeTrendPayload('SHOPEECN',r,force),shopeeTrendPayload('SHOPEEVN',r,force)
      ]);
      if(id!==requestId)return;applyHomeCurrent(current);renderTrends(allTrend);renderHomeAttempts(cnTrend,vnTrend);applyHomeDispatchTruth(cnTrend,vnTrend);removeDuplicateHomeAssessment(homeRoot());scheduleCurrentRetry(current);scheduleTrendRetry(allTrend);scheduleLedgerRetry(cnTrend,vnTrend);
  ...
  console.info('[CE-QC][V247_HOME_DASHBOARD_OWNER]',VERSION,'home removes duplicate same-day assessment and reads Shopee attempts/regions from V246 locked tracking ledger');
})(window);