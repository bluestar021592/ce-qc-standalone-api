(function installRequestCoalescingV65(global) {
  ...
  const VERSION = '2026-08-12-v65-request-coalescing-v1';
  ...
  document.addEventListener('click', event => {
    if (event.target?.closest?.('#topRangeQuery,#dashboardRangeQuery,[data-page]')) clearRecent();
  }, true);
  document.addEventListener('change', event => {
    if (event.target?.matches?.('#topRangeFrom,#topRangeTo,#dashboardRangeFrom,#dashboardRangeTo')) clearRecent();
  }, true);
  global.addEventListener('popstate', clearRecent);
