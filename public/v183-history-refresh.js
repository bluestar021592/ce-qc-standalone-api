(function installV183HistoryRefresh(global) {
  ...
  function refreshUiSoon() { setTimeout(() => { ensurePanel(); void readSummary(false); }, 0); }
  for (const id of ['periodExportDate','periodExportFrom','periodExportTo','periodExportBusiness']) document.getElementById(id)?.addEventListener('change', refreshUiSoon);
  document.querySelectorAll('.period-tab').forEach(button => button.addEventListener('click', refreshUiSoon));
  if (typeof global.renderReportsPage === 'function') {
    const originalRender = global.renderReportsPage;
    global.renderReportsPage = function v183RenderReportsPage(...args) { const result = originalRender.apply(this, args); refreshUiSoon(); return result; };
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', refreshUiSoon, { once:true }); else refreshUiSoon();
  console.info('[CE-QC][V183_HISTORY_REFRESH_UI]', VERSION);
})(window);
