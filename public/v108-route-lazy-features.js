(function installRouteLazyFeaturesV108(global){
  ...
  const VERSION='2026-08-14-v108-route-lazy-features-v1';
  ...
  const groups={
    ...
    import:[
      '/v66-import-success-whpp.js?v=20260812-1','/v96-v67-live-progress-bridge.js?v=20260813-1'
    ],
    reports:['/v84-async-export-ui.js?v=20260814-4'],
    settings:['/v62-network-settings-runtime.js?v=20260812-1'],
    data:['/v104-fast-purge-ui.js?v=20260814-4','/v106-purge-legacy-controls-hide.js?v=20260814-1'],
  ...
  function loadScript(src){
    ...
    const promise=new Promise((resolve,reject)=>{
      ...
      script.src=src;script.async=false;script.dataset.ceQcLazy='v108';
      script.onload=()=>resolve(script);script.onerror=()=>reject(new Error(`加载页面功能失败：${src}`));
  ...
  function warmIdle(){
    const work=async()=>{
      for(const name of ['shopee','import','reports','settings','data','carry']){
        try{await loadGroup(name);}catch{}
        await new Promise(resolve=>setTimeout(resolve,0));
      }
    };
    if('requestIdleCallback'in global)global.requestIdleCallback(()=>void work(),{timeout:15000});
    else setTimeout(()=>void work(),8000);
  }
