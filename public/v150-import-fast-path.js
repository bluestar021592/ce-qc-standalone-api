(function installUnifiedImportFastPathV150(global){
  ...
  const VERSION='2026-08-15-v150-import-fast-path-v3';
  let busy=false,catalogTimer=null;

  const el=id=>document.getElementById(id);
  const html=value=>String(value??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
  const fmt=value=>Number(value||0).toLocaleString('zh-CN');
  function setBusy(value){busy=Boolean(value);const button=document.querySelector('[data-testid="combined-daily-import"]');if(button){button.disabled=busy;button.textContent=busy?'正在保存日报…':'导入综合日报并自动分类';}const file=el('excelFile');if(file)file.disabled=busy;}
  ...
  function bindState(result){try{unifiedImportState=result;}catch{global.unifiedImportState=result;}try{if(result.state)appState=result.state;}catch{}try{if(result.shopeeState)shopeeState=result.shopeeState;}catch{}try{historyModeDate=result.reportDate||historyModeDate;}catch{}try{reportDateManualCorrection=false;reportDateOverrideSource='';}catch{}const date=el('reportDate');if(date&&result.reportDate){date.value=result.reportDate;date.readOnly=true;}const source=el('dateDetectionSource');if(source)source.textContent=result.dateDetectionSource||'日报自动识别';try{if(typeof renderUnifiedImportResult==='function')renderUnifiedImportResult();}catch(error){console.warn('[CE-QC][V150_IMPORT] result render skipped',error);}}
  ...
  async function importUnifiedExcelFast(){
    ...
    setBusy(true);setStatus(`正在保存 ${html(file.name)}，保存完成后会立即释放页面…`);
    const body=new FormData();body.append('file',file);const date=el('reportDate');if(date&&!date.readOnly&&date.value)body.append('reportDate',date.value);
    try{
      const response=await fetch('/api/import/unified-daily-report',{method:'POST',body,cache:'no-store',credentials:'same-origin'});
      const result=await readJson(response);bindState(result);setStatus(successText(result),'success');if(input)input.value='';refreshCatalogLater();
  ...
  global.__CE_QC_V150_IMPORT_FAST_PATH__={version:VERSION,import:importUnifiedExcelFast,policy:'SAVE_FIRST_COALESCE_BACKGROUND_REFRESH'};
