(function installUnifiedImportFastPathV150(global){
  ...
  const VERSION='2026-08-15-v150-import-fast-path-v2';
  let busy=false;
  ...
  async function readJson(response){
    ...
    if(!response.ok||payload?.ok===false){const error=new Error(payload?.error||payload?.message||`上传失败（HTTP ${response.status}）`);error.status=response.status;error.payload=payload;throw error;}
    return payload;
  }
  ...
  async function importUnifiedExcelFast(){
    ...
    try{
      ...
      setStatus(`<b>导入失败：${html(error?.message||error)}</b>${detail}`,'danger');return null;
    }finally{setBusy(false);}
  }
  global.importUnifiedExcel=importUnifiedExcelFast;
  global.__CE_QC_V150_IMPORT_FAST_PATH__={version:VERSION,import:importUnifiedExcelFast,policy:'SAVE_FIRST_DEFER_DASHBOARD_REFRESH'};
