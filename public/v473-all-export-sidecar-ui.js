(function installV473AllExportSidecarUi(global){
  ...
  async function ensureSidecar(base,progress){
    await json('/api/export-sidecar/start',{method:'POST',headers:{'content-type':'application/json'},body:'{}'},3000,'独立导出服务启动');
    let lastError=null;
    for(let attempt=1;attempt<=12;attempt+=1){
      try{
        const ping=await json(`${base}/api/v473/export-ping`,{},1400,'V473独立导出服务');
        if(String(ping.version||'')!==SIDECAR_VERSION)throw new Error(`V473独立导出服务版本不一致：${ping.version||'未知版本'}`);
        return ping;
      }catch(error){
        lastError=error;
        if(progress)progress.textContent=`[V473独立导出] 正在按需启动5178独立进程… ${attempt}/12`;
        await sleep(500);
      }
    }
    throw lastError||new Error('V473独立导出服务启动超时');
  }
  ...
  async function resume(){const active=loadActive();if(!active)return;const {progress,files}=targets();if(!progress||!files)return;progress.textContent=`[V473独立导出] 检测到任务 ${active.jobId}，正在恢复5178状态通道…`;try{const base=sidecarOrigin();if(base)await ensureSidecar(base,progress);await waitForJob(active.jobId,active.pollUrl,progress,files);}catch(error){if(Number(error.status||0)===404)clearActive();progress.textContent=`[V473独立导出] 恢复任务失败：${error?.message||String(error)}`;}}
  ...
  console.info('[CE-QC][V473_EXPORT_UI]',VERSION,'5178 export sidecar starts on explicit export/resume demand; ordinary app startup stays single-process-first.');
})(window);
