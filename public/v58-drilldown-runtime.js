(function installDrilldownRuntimeV58(global){
  ...
  function cardLabel(card){
    return canonicalLabel(card?.querySelector('.v18-metric-label')?.textContent||card?.querySelector('span')?.textContent||card?.dataset?.label||'');
  }
  ...
  function normalizeType(value=''){
    ...
    if(raw==='SHOPEECN')return'SHOPEECN';
    if(raw==='SHOPEEVN')return'SHOPEEVN';
    return'';
  }
  ...
  function pageBusinessType(card){
    ...
    try{
      const state=global.currentBusinessState?.();
      const current=normalizeType(state?.businessType||state?.dashboard?.businessType||'');
  ...
  function columns(rows){
    ...
    if(found.length)return found.slice(0,13);
    return Object.keys(rows[0]||{}).slice(0,13);
  }
  ...
  async function syncSevereMetric(){
    ...
    try{
      ...
      document.querySelectorAll('.v18-core-grid .v18-metric-card').forEach(card=>{
        const label=cardLabel(card);if(label!=='严重异常')return;
        const value=Number(summary.severe??summary.severeAbnormal??summary.abnormal??0),total=Number(summary.total||0);
        const b=card.querySelector('b'),small=card.querySelector('small');if(b)b.textContent=fmt(value);if(small)small.textContent=total?`占本业务 ${(value*100/total).toFixed(2)}%`:'占本业务 0.00%';
      });
    }catch{}
  }
  ...
  function onClick(event){
    ...
    void openDetail(type,tab,label);
  }
