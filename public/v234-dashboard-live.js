(function installV234DashboardLive(global){
  ...
  const VERSION='2026-08-22-v234-dashboard-live-client-v2';
  ...
  function setPendingCard(card){if(!card)return;const b=card.querySelector('b'),e=card.querySelector('em'),s=card.querySelector('small');if(b)b.textContent='—';if(e)e.textContent='等待正式快照';if(s)s.textContent='状态读取中';}
  ...
  function applyCurrent(payload,t=type()){
    ...
    if(!m.ready){
      notice(root,`${t} 当前日报状态缓存尚未完成；系统不会把“总票-POD-退回”直接伪装成全部未闭环。正式快照完成后自动更新。`);
      cards.forEach((card,index)=>{if(index>0)setPendingCard(card);});
      root.querySelectorAll('.v18-core-grid .v18-metric-card').forEach(card=>{const b=card.querySelector('b'),s=card.querySelector('small');if(b)b.textContent='—';if(s)s.textContent='等待正式快照 · 点击暂不读取';});
      return;
    }
