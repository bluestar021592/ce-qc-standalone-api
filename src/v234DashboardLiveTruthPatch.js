...
function cacheRows(reportDate,snapshotId=''){
  const db=getDb();const params=[reportDate];let clause='';if(snapshotId){clause=' AND snapshotId=?';params.push(snapshotId);}
...
function trendPayload(type,from,to){
  ...
  for(const date of dates){if(type==='WHPP'){daily.push(whppSummary(date));continue;}const batch=latestBatch(date);const rows=cacheRows(date,batch?.snapshotId||'');const wanted=type==='ALL'?TYPES:[type];const out=blank(type,date);for(const row of rows){if(!wanted.includes(String(row.businessType||'').toUpperCase()))continue;addMetric(out,safeJson(row.metricsJson));out.ready=true;}if(type==='ALL'){const w=whppSummary(date);if(w.ready){addMetric(out,w);out.ready=true;}}daily.push(finish(out));}
...
function metricCondition(type,tab){
  const shopee=type.startsWith('SHOPEE');const clean=String(tab||'all').replace(/^(ALL|CN|VN|OTHER)_/,'');
...
function detailPayload({type,from,to,tab,page,pageSize}){
  ...
  const params=[from,to,type];const count=n(db.prepare(`${base} SELECT COUNT(*) AS c FROM base f WHERE ${condition}`).get(...params)?.c);
...
express.response.send=function v234Inject(body){if(typeof body==='string'&&body.includes('</body>')&&body.includes('CE Express')){const chart='/dashboard-chart-v18.js?v=20260822-v234-1',client='/v234-dashboard-live.js?v=20260822-v234-1';const tags=[];if(!body.includes(chart))tags.push(`  <script src="${chart}"></script>`);if(!body.includes(client))tags.push(`  <script src="${client}"></script>`);if(tags.length)body=body.replace('</body>',`${tags.join('\n')}\n</body>`);this.setHeader?.('X-CE-QC-V234',V234_DASHBOARD_LIVE_TRUTH_ID);}return originalSend.call(this,body);};
