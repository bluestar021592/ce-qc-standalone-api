...
export const V236_DASHBOARD_CURRENT_READ_ID='2026-08-22-v238-terminal-safe-current-read-v1';
...
function addMetric(out,m={}){
  ...
  out.pendingNonContinuous+=firstNumber(m,['pendingNonContinuous','pendingGap']);out.oc1+=firstNumber(m,['oc1']);out.oc2+=firstNumber(m,['oc2']);out.oc3+=firstNumber(m,['oc3','oc3plus']);
...
function importedCounts(snapshotId=''){if(!snapshotId)return{};return Object.fromEntries(getDb().prepare('SELECT businessType,COUNT(*) AS c FROM unified_import_rows WHERE snapshotId=? GROUP BY businessType').all(snapshotId).map(row=>[String(row.businessType||'').toUpperCase(),n(row.c)]));}
...
function zeroRow(type,snapshotId){return{businessType:type,regionCode:'',metricsJson:JSON.stringify({total:0,pod:0,returned:0,cancelled:0,pending1:0,pending2:0,pending3:0,pendingNonContinuous:0,oc1:0,oc2:0,oc3:0,cycle2:0,inboundNoScan:0,delivery1:0,deliveryStay:0,provinceOpen:0,attempt1:0,attempt2:0,attempt3:0}),snapshotId,refreshedAt:'DIRECT_NORMALIZED'};}
...
function directRows(date,snapshotId,counts){const rows=[...directCcslRows(date,snapshotId),...directShopeeRows(date,snapshotId)];const present=new Set(rows.map(row=>String(row.businessType||'').toUpperCase()));for(const type of V236_TYPES)if(!present.has(type)&&n(counts[type])===0)rows.push(zeroRow(type,snapshotId));return rows;}
...
function whpp(date){const out=blank('WHPP',date);let payload={};try{const row=getDb().prepare("SELECT totalCount,summaryJson FROM business_daily_reports WHERE businessType='WHPP' AND reportDate=? LIMIT 1").get(date);if(row){payload={...safeJson(row.summaryJson)};if(payload.total===undefined)payload.total=n(row.totalCount);}}catch{}try{const row=getDb().prepare("SELECT summaryJson FROM business_history_summary WHERE businessType='WHPP' AND reportDate=? LIMIT 1").get(date);if(row)payload={...payload,...safeJson(row.summaryJson)};}catch{}addMetric(out,payload);out.ready=out.total>0||Object.keys(payload).length>0;return finish(out);}
...
export function readV236CurrentSummary(reportDate='',options={}){
  const cacheOnly=Boolean(options?.cacheOnly);const batch=latestBatch(reportDate),date=batch?.reportDate||String(reportDate||'').slice(0,10);const key=`${cacheOnly?'CACHE':'CURRENT'}|${date}|${batch?.snapshotId||''}|${batch?.snapshotStatus||''}`;const hit=summaryCache.get(key);if(hit&&Date.now()-hit.at<SUMMARY_CACHE_MS)return hit.value;
  const counts=importedCounts(batch?.snapshotId||'');let rows=[],source='IMPORT_ONLY';
  if(batch?.snapshotId){rows=exactCacheRows(date,batch.snapshotId);if(rowsReadyForAllTypes(rows))source='EXACT_DASHBOARD_CACHE';else if(!cacheOnly&&normalizedDashboardCoverageReady(batch)){rows=directRows(date,batch.snapshotId,counts);source='DIRECT_NORMALIZED_FULL_COVERAGE';}else rows=[];}
