...
export const V480_FIRST_PAINT_STATIC_ID='2026-09-08-v480-preauth-static-first-paint-v1';
const originalUse = express.application.use;
const previousPost = express.application.post;
let installed = false;
let v480FirstPaintInstalled = false;
...
const V480_PUBLIC_ASSET_RE=/\.(?:css|js|svg|png|jpe?g|webp|gif|ico|woff2?)$/i;
const v480PublicStatic=express.static('public',{index:false,fallthrough:true,redirect:false,maxAge:0});

function v480FirstPaintAsset(req,res,next){
  const pathname=String(req.path||req.url||'').split('?')[0];
  if(!V480_PUBLIC_ASSET_RE.test(pathname))return next();
  res.setHeader('Cache-Control','no-store, max-age=0, must-revalidate');
  res.setHeader('Pragma','no-cache');
  res.setHeader('Expires','0');
  res.setHeader('X-CE-QC-V480-First-Paint','PUBLIC_STATIC_ONLY');
  return v480PublicStatic(req,res,next);
}
...
express.application.use = function v280StaticAssetCacheUse(...args) {
  ...
  if (!v480FirstPaintInstalled && candidates.some(fn => fn.name === 'accessIdentity')) {
    v480FirstPaintInstalled = true;
    originalUse.call(this, v480FirstPaintAsset);
  }
...
console.info('[CE-QC][V280_IMPORT_FAST_ACK]', PATCH_ID, V480_FIRST_PAINT_STATIC_ID, 'native SPA preserved; public non-HTML first-paint assets bypass DB-backed session middleware; HTML/API auth remains mandatory; upload/census/parse/COMMIT timing observable; compatibility saves continue in background.');
