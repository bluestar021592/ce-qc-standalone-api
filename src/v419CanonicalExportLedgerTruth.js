...
function applyTerminalTruth(type,row,ledger){
  ...
  if(reason==='POD'){
    ...
    const lockedPodDate=dateKey(ledger.podDate),recoveredPodDate=lockedPodDate?'':scan85SavedPodDate(ledger),podDate=lockedPodDate||recoveredPodDate;
    row.podDate=podDate;if(podDate)row.podTime=dateKey(legacyPodTime)===podDate?legacyPodTime:podDate;
    if(recoveredPodDate){row.podSource='V493_SCAN85_SAVED_TERMINAL_TIME';row.v493Scan85PodDateRecoveryId=V493_SCAN85_POD_DATE_RECOVERY_ID;}
    ...
    if(STRICT_TYPES.has(type)){
      ...
      if(strictDays>0){row.signingDays=strictDays;row.deliveryDays=strictDays;row.signingDaysSource=recoveredPodDate?'V493保存的85终态时间 + V246严格START→POD':'V246严格START→POD';row.deliveryDaysSource=row.signingDaysSource;row.dispatchSigningEvidenceComplete=true;row.signingEvidenceComplete=true;}
