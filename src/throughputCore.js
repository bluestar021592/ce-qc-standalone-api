export const THROUGHPUT_CORE_ID='system-throughput-core-v1';
// Compatibility identifiers kept until old gates/imports are removed.
...
export function splitFixedBatches(values=[],size=50){
  ...
  const out=[];
  for(let i=0;i<codes.length;i+=width)out.push(codes.slice(i,i+width));
  return out;
}

function batchKey(values=[]){return cleanShipmentCodes(values).join('\u001f');}
function successfulBills(statusRows=[]){
  return new Set((statusRows||[])
    .filter(row=>['success','skipped_pod','skipped_return'].includes(String(row?.status||'').toLowerCase()))
...
function planBatches(sourceBills=[],completed=new Set(),width=50,mode='stable-filter'){
  ...
  return splitFixedBatches(source,width).map(batch=>batch.filter(code=>!completed.has(code))).filter(batch=>batch.length);
}
function runWithBudget(query,batch,budgetMs){
  const budget=Number(budgetMs||0);
  if(!(budget>0))return Promise.resolve().then(()=>query(batch));
  let timer;
  const timeout=new Promise((_,reject)=>{
    timer=setTimeout(()=>{
      const error=new Error(`confirm query exceeded ${budget}ms`);
      error.code='CCSL_CONFIRM_PREFETCH_HARD_TIMEOUT';
      reject(error);
    },budget);
  });
...
export function createBoundedPrefetchPool({
  ...
  function pump(){
    while(active<limit&&queue.length){
      ...
      active+=1;
      runWithBudget(query,job.batch,hardBudgetMs)
  ...
  function enqueue(batch,priority=false){
    ...
    cache.set(key,entry);
    const job={key,batch:clean,finish};
  ...
  function launch(focusBatch=[]){
    ...
    if(focusKey&&plannedKeys.has(focusKey)){
      batches=[...batches.filter(batch=>batchKey(batch)===focusKey),...batches.filter(batch=>batchKey(batch)!==focusKey)];
    }
    console.info('[CE-QC][THROUGHPUT_PREFETCH]',JSON.stringify({
      core:THROUGHPUT_CORE_ID,label,batchSize:width,concurrency:limit,batches:batches.length,
      completedBills:completed.size,planMode,hardBudgetMs:Number(hardBudgetMs||0),allRequestsBounded:true
    }));
    for(const batch of batches)enqueue(batch,false);
  }
  ...
  async function request(codes=[]){
    ...
    if(!settled.ok){cache.delete(key);throw settled.error;}
    return settled.value;
  }

  return{
    label,batchSize:width,concurrency:limit,planMode,request,
    stats:()=>({active,queued:queue.length,cached:cache.size,planned:plannedKeys.size,launched,batchSize:width,concurrency:limit,hardBudgetMs:Number(hardBudgetMs||0),allRequestsBounded:true})
  };
}

export function createUnifiedThroughputClient(state={},client,options={}){
  if(!client||typeof client!=='object')return client;
  const forced=String(options.businessType||'').toUpperCase();
  const isShopee=(forced||String(state.businessType||'').toUpperCase())==='SHOPEE';
  const pools={};
  const label=forced||String(state.businessType||'').toUpperCase()||(isShopee?'SHOPEE':'CCSL');
  ...
  if(confirmQuery)pools.confirm=createBoundedPrefetchPool({
    getSourceBills:()=>state.scanPool||state.pnhBills||[],
    getStatusRows:()=>state.scanQueryStatus||[],
    batchSize:350,
    concurrency:isShopee?(options.confirmConcurrency||V314_CONFIRM_CONCURRENCY):(options.confirmConcurrency||V339_CCSL_CONFIRM_CONCURRENCY),
    query:confirmQuery,
    label:`${label}-confirm-350`,
    planMode:options.confirmPlanMode||((isShopee||forced==='WHPP')?'compact-pending':'stable-filter'),
    hardBudgetMs:isShopee?0:(options.hardBudgetMs||V339_CCSL_CONFIRM_HARD_BUDGET_MS)
  });
  if(trackQuery)pools.track=createBoundedPrefetchPool({
    getSourceBills:()=>state.needTrackBills||[],
    getStatusRows:()=>isShopee?(state.eventQueryStatus||[]):(state.trackQueryStatus||state.eventQueryStatus||[]),
    batchSize:50,
    concurrency:options.trackConcurrency||options.eventConcurrency||V314_EVENT_CONCURRENCY,
    query:trackQuery,
    label:`${label}-track-50`,
    planMode:isShopee?'stable-filter':'compact-pending'
  });
  if(exceptionQuery)pools.exception=createBoundedPrefetchPool({
    getSourceBills:()=>state.needExceptionBills||state.needTrackBills||[],
    getStatusRows:()=>state.exceptionQueryStatus||[],
    batchSize:50,
    concurrency:options.exceptionConcurrency||V314_EXCEPTION_CONCURRENCY,
    query:exceptionQuery,
    label:`${label}-exception-50`,
    planMode:'stable-filter'
  });
  ...
  return new Proxy(client,{
    get(target,prop){
      ...
      if(prop==='trackQuery'&&pools.track)return codes=>pools.track.request(codes);
      if(prop==='exceptionQuery'&&pools.exception)return codes=>pools.exception.request(codes);
      if(prop==='__v314Pools')return pools;
      if(prop==='__v339CcslConfirmPool')return pools.confirm;
      if(prop==='__ceQcThroughputPools')return pools;
...
export function createShopeeThroughputClient(state={},client,options={}){
  return createUnifiedThroughputClient(state,client,{...options,businessType:'SHOPEE'});
}
export function createCcslThroughputClient(state={},client,options={}){
  return createUnifiedThroughputClient(state,client,{...options,businessType:'CCSL'});
}
...
console.info('[CE-QC][CORE_THROUGHPUT]',JSON.stringify({
  id:THROUGHPUT_CORE_ID,scanBatchSize:350,trackBatchSize:50,trackConcurrency:V314_EVENT_CONCURRENCY,
  exceptionConcurrency:V314_EXCEPTION_CONCURRENCY,shopeeConfirmConcurrency:V314_CONFIRM_CONCURRENCY,
  ccslConfirmConcurrency:V339_CCSL_CONFIRM_CONCURRENCY,
  policy:'ONE_UNVERSIONED_THROUGHPUT_OWNER_ALL_BUSINESSES_SCAN_350_TRACK_50_X4'
}));
