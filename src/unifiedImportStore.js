...
const STABLE_CLOSED_STATES = Object.freeze(['POD','RETURNED','RETURN_COMPLETED','ORDER_CANCELLED','SELF_PICKUP','CECN_RETENTION','CEZT_RETENTION','CCSL580_RETENTION','CCSLCN_DIVERSION','CCSLZT_DIVERSION','CCSL580_DIVERSION','NORMAL_FINAL']);
const STABLE_CLOSED_SQL = STABLE_CLOSED_STATES.map(value => `'${value}'`).join(',');
...
export function saveUnifiedImport(parsed, sourceName) {
  ...
  try {
    ...
    const upsertCurrent = db.prepare(`INSERT INTO shipment_current_state(shipmentCode,businessType,reportDate,snapshotId,state,apiStatus,lastEventTime,stateJson,updatedAt) VALUES(?,?,?,?,?,'PENDING_SCAN','',?,?) ON CONFLICT(shipmentCode) DO UPDATE SET businessType=excluded.businessType,reportDate=excluded.reportDate,snapshotId=excluded.snapshotId,state=CASE WHEN UPPER(COALESCE(shipment_current_state.state,'')) IN (${STABLE_CLOSED_SQL}) THEN shipment_current_state.state ELSE 'PENDING_SCAN' END,apiStatus=CASE WHEN UPPER(COALESCE(shipment_current_state.state,'')) IN (${STABLE_CLOSED_SQL}) THEN shipment_current_state.apiStatus ELSE 'PENDING_SCAN' END,lastEventTime=CASE WHEN UPPER(COALESCE(shipment_current_state.state,'')) IN (${STABLE_CLOSED_SQL}) THEN shipment_current_state.lastEventTime ELSE '' END,stateJson=CASE WHEN UPPER(COALESCE(shipment_current_state.state,'')) IN (${STABLE_CLOSED_SQL}) THEN shipment_current_state.stateJson ELSE excluded.stateJson END,updatedAt=excluded.updatedAt`);
    const upsertCarry = db.prepare(`INSERT INTO carryover_open_items(shipmentCode,businessType,sourceReportDate,lastReportDate,sourceSnapshotId,lastSnapshotId,status,apiStatus,closeReason,stateJson,createdAt,updatedAt) VALUES(?,?,?,?,?,?,'OPEN','PENDING_SCAN','',?,?,?) ON CONFLICT(shipmentCode) DO UPDATE SET businessType=excluded.businessType,lastReportDate=excluded.lastReportDate,lastSnapshotId=excluded.lastSnapshotId,status=CASE WHEN carryover_open_items.status='CLOSED' AND UPPER(COALESCE(carryover_open_items.closeReason,'')) IN (${STABLE_CLOSED_SQL}) THEN 'CLOSED' ELSE 'OPEN' END,apiStatus=CASE WHEN carryover_open_items.status='CLOSED' AND UPPER(COALESCE(carryover_open_items.closeReason,'')) IN (${STABLE_CLOSED_SQL}) THEN carryover_open_items.apiStatus ELSE 'PENDING_SCAN' END,closeReason=CASE WHEN carryover_open_items.status='CLOSED' AND UPPER(COALESCE(carryover_open_items.closeReason,'')) IN (${STABLE_CLOSED_SQL}) THEN carryover_open_items.closeReason ELSE '' END,stateJson=CASE WHEN carryover_open_items.status='CLOSED' AND UPPER(COALESCE(carryover_open_items.closeReason,'')) IN (${STABLE_CLOSED_SQL}) THEN carryover_open_items.stateJson ELSE excluded.stateJson END,updatedAt=excluded.updatedAt`);
...
export function updateCarryoverResults({ snapshotId, reportDate, rows = [] }) {
  ...
  try {
    for (const row of rows) {
      ...
      if (!bill) continue;
      const pod = row.是否POD === '是' || String(row.orderStatus || '') === '85' || String(row.currentState || row.scanNormalizedState || '').toUpperCase() === 'POD';
      const returned = row.退回状态 === '已退回' || ['RETURNED','RETURN_COMPLETED'].includes(String(row.currentState || row.scanNormalizedState || '').toUpperCase()) || /退回|RETURN/i.test(String(row.primaryCategory || row.主分类 || ''));
      const cancelled = !pod && !returned && (String(row.orderStatus || '') === '10' || String(row.currentState || row.scanNormalizedState || '').toUpperCase() === 'ORDER_CANCELLED' || row.订单取消 === '是' || row.取消状态 === '已取消' || /订单取消|取消订单|已取消|ORDER_CANCELLED|CANCELLED|CANCELED/i.test(String(row.primaryCategory || row.主分类 || row.异常分类 || '')));
      const specialState = String(row.specialState || row.primaryCategory || row.主分类 || '').trim().toUpperCase();
      const specialClosed = ['SELF_PICKUP', 'CECN_RETENTION', 'CEZT_RETENTION', 'CCSL580_RETENTION', 'CCSLCN_DIVERSION', 'CCSLZT_DIVERSION', 'CCSL580_DIVERSION'].includes(specialState) || row.主分类 === '仓库自提';
      ...
      const closed = pod || returned || cancelled || specialClosed || normal;
      const status = closed ? 'CLOSED' : 'OPEN';
      const reason = pod ? 'POD' : returned ? 'RETURNED' : cancelled ? 'ORDER_CANCELLED' : specialClosed ? (specialState || 'SELF_PICKUP') : normal ? 'NORMAL_FINAL' : '';
