...
export const V375_UNIFIED_IMPORT_METADATA_ID='2026-08-31-v375-snapshot-metadata-hydration-v1';
...
export function readV375LatestUnifiedImport(db=getDb()){
  ...
  const payloadValid=Number(payloadSummary?.validUniqueWaybills||0);
  ...
  const derivedRegions={PP:0,PV:0};
  for(const item of regionRows){if(item.regionCode==='PP')derivedRegions.PP+=Number(item.count||0);else if(item.regionCode==='PV')derivedRegions.PV+=Number(item.count||0);}
  const candidateRegions={PP:Number(storedRegions.PP??payloadRegions.PP??0),PV:Number(storedRegions.PV??payloadRegions.PV??0)};
  const regionCounts=(candidateRegions.PP+candidateRegions.PV>0||derivedRegions.PP+derivedRegions.PV===0)?candidateRegions:derivedRegions;
...
function handler(req,res){
  try{return res.json({ok:true,import:readV375LatestUnifiedImport()});}
...
express.application.get=function v375UnifiedImportMetadataGet(route,...handlers){
  if(route==='/api/import/unified-latest')return originalGet.call(this,route,handler);
  return originalGet.call(this,route,...handlers);
};

console.info('[CE-QC][V375_IMPORT_METADATA]',V375_UNIFIED_IMPORT_METADATA_ID,'latest VALID import metadata is hydrated from its own snapshot payload and persisted row regions; no database writes or schema changes.');
