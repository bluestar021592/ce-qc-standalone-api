...
function empty(date) {
  return { date, total:0, pod:0, returned:0, pending:0, delivering:0, activeNotPod:0, a1:0, a2:0, a3:0, attemptUnknown:0, signingDays:[], deliveryDays:[] };
}
...
function finish(stat) {
  ...
  return {
    ...
    attemptDenominator: podBase,
    attempt1Rate: pct(stat.a1, podBase),
    attempt2Rate: pct(stat.a2, podBase),
    attempt3Rate: pct(stat.a3, podBase),
...
async function run(input = {}) {
  ...
  for (const type of types) {
    ...
    for (const row of rows) allRows.push({ ...row, __metricBusinessType: type });
  }
  ...
  return {
    ...
    formula: {
      ...
      attemptRate: '1/2/3派占比 = 对应派次POD件数 ÷ 当日已POD件数。',
