...
export const V207_EXPORT_MEMBERSHIP_VERSION='2026-08-19-v207-export-membership-ledger-v1';
...
export async function collectV207ExportRows(type,range,onProgress=()=>{}){
  ...
  if(!ownership.length){if(base.length)return base;if(baseError)throw new Error(baseError);return[];}
  ...
  onProgress({phase:'v207ExportMembership',completed:official.length,total:official.length,businessType,ownershipRows:wanted.size,recoveredMissingRows:recovered,manualEvidenceRows:manual.length,previousWarning:baseError,version:V207_EXPORT_MEMBERSHIP_VERSION});
