...
export function readV253DashboardTrends(businessType='ALL',fromDate='',toDate=''){
  ...
  if(from===to){const summary=readV236CurrentSummary(to);const row=currentMetric(type,to,summary);result=trendPayload(type,from,to,[row],'PER_BUSINESS_SINGLE_DAY_FIRST_PAINT_NO_HISTORY_SCAN');}
    ...
    const canonical=readV284DashboardTrends(type,from,to);
    let daily=Array.isArray(canonical.daily)?canonical.daily:[];
    if(type==='WHPP'||type==='ALL')daily=validateWhppRangeMembership(type,daily);
    daily=daily.filter(row=>type==='WHPP'||n(row.total)>0).map(row=>finish(row));
    result=trendPayload(type,from,to,daily,'PER_BUSINESS_EXPLICIT_RANGE');result.canonicalTruthId=canonical.id;
  }
...
export function invalidateV253DashboardFastPath(){memory.clear();}
globalThis.__CE_QC_INVALIDATE_V253_DASHBOARD_FAST_PATH__=invalidateV253DashboardFastPath;