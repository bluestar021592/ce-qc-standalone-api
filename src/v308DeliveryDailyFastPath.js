...
function strictDailyEvidence(type,date,db){
  ...
  try{return db.prepare(`WITH facts AS (
    SELECT CASE WHEN UPPER(TRIM(COALESCE(u.regionCode,'')))='PP' THEN 'PP' WHEN UPPER(TRIM(COALESCE(u.regionCode,'')))='PV' THEN 'PV' ELSE 'UNKNOWN' END regionCode,
      l.terminalReason,l.attemptNo,l.attemptSource,l.firstReportDate,l.podDate
    ...
    SELECT *,CASE WHEN terminalReason='POD' AND firstDay<>'' AND podDay<>'' AND julianday(podDay)>=julianday(firstDay) THEN CAST(julianday(podDay)-julianday(firstDay)+1 AS INTEGER) ELSE 0 END signingDays FROM normalized
  ) SELECT
