...
function strictDailyEvidence(type,date,db){
  const snapshotId=latestSnapshotId(db,date);if(!snapshotId)return{attempt1:0,attempt2:0,attempt3:0,signingDaysSum:0,signingDaysCount:0};
...
function dailyHandler(req,res){try{const started=Date.now(),historyAll=String(req.query.history||'').toLowerCase()==='all',type=String(req.query.businessType||'').toUpperCase(),from=dateKey(req.query.from),to=dateKey(req.query.to),needsHistory=historyAll||Boolean(from&&to&&from!==to),repair=needsHistory&&TYPES.has(type)&&to?requestV328EvidenceRepair(type,to):inspectV328EvidenceRepair(type),data=readV308DeliveryDaily(type,from,to,getDb(),{historyAll});data.evidenceRepair=repair;res.setHeader('Cache-Control','private,max-age=5');res.setHeader('X-CE-QC-V308',V308_DELIVERY_DAILY_FAST_ID);res.setHeader('X-CE-QC-V320',V320_HISTORICAL_DAILY_TRUTH_ID);res.setHeader('X-CE-QC-V329',V329_THREE_BUSINESS_DAILY_CACHE_ID);res.setHeader('Server-Timing',`v329daily;dur=${Date.now()-started}`);return res.json(data);}catch(error){return res.status(400).json({ok:false,id:V308_DELIVERY_DAILY_FAST_ID,error:error?.message||String(error)});}}
