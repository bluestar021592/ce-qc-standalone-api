...
export const V308_DELIVERY_DAILY_FAST_ID='2026-08-27-v335-three-business-per-business-current-v1';
...
function strictDailyEvidence(type,date,db){
  ...
  FROM normalized`).get(type,snapshotId,type)||{};}catch(error){console.warn('[CE-QC][V335_DELIVERY_EVIDENCE_FAST_READ_FAILED]',type,date,error?.message||error);return{attempt1:0,attempt2:0,attempt3:0,signingDaysSum:0,signingDaysCount:0};}
}
function singleDay(type,date,db){
  // V235 legacy cache may still be bound to another business's later same-date snapshot. V236 validates exact snapshot ownership and, when needed, falls back to this business's own normalized daily facts.
...
function dailyHandler(req,res){try{const started=Date.now(),historyAll=String(req.query.history||'').toLowerCase()==='all',type=String(req.query.businessType||'').toUpperCase(),from=dateKey(req.query.from),to=dateKey(req.query.to),needsHistory=historyAll||Boolean(from&&to&&from!==to),repair=needsHistory&&TYPES.has(type)&&to?requestV328EvidenceRepair(type,to):inspectV328EvidenceRepair(type),db=getDb(),data=needsHistory&&repair?.cacheBlocked?blockedHistory(type,from,to,db):readV308DeliveryDaily(type,from,to,db,{historyAll});data.evidenceRepair=repair;res.setHeader('Cache-Control','private,max-age=5');res.setHeader('X-CE-QC-V308',V308_DELIVERY_DAILY_FAST_ID);res.setHeader('X-CE-QC-V320',V320_HISTORICAL_DAILY_TRUTH_ID);res.setHeader('X-CE-QC-V329',V329_THREE_BUSINESS_DAILY_CACHE_ID);res.setHeader('Server-Timing',`v335daily;dur=${Date.now()-started}`);return res.json(data);}catch(error){return res.status(400).json({ok:false,id:V308_DELIVERY_DAILY_FAST_ID,error:error?.message||String(error)});}}
...
function register(app){if(!isRealApp(app)||registeredApps.has(app))return false;registeredApps.add(app);previousGet.call(app,'/api/v308/delivery-daily',dailyHandler);previousGet.call(app,'/api/v328/evidence-status',statusHandler);console.info('[CE-QC][V335_DELIVERY_DAILY]',V308_DELIVERY_DAILY_FAST_ID,'TBKH/CN/VN single-day truth validates exact per-business cache ownership and falls back to that business own normalized daily facts; history remains cache-only on the web process.');return true;}
express.application.get=function v335DeliveryDailyRoute(pathValue,...handlers){const path=typeof pathValue==='string'?pathValue:'';if(path.startsWith('/')&&!['/api/v308/delivery-daily','/api/v328/evidence-status'].includes(path))register(this);return previousGet.call(this,pathValue,...handlers);};