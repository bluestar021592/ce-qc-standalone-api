...
export const V308_DELIVERY_DAILY_FAST_ID='2026-08-26-v320-shopee-history-dispatch-signing-v2';
...
export function readV308DeliveryDaily(businessType='',fromDate='',toDate='',db=getDb()){
  ...
  const data=readV320HistoricalDaily(type,fromDate,toDate,{db,expandSingle:true});
  ...
  return{
    ...
    source:'V320_PERSISTED_HISTORY + REAL_DISPATCH_START_TO_POD_SAMPLE_AVERAGE',
    definitions:{
      ...
      averageDays:'平均派件→签收天数只使用同时有真实首次派件START/firstAttemptAt和真实POD日期的票，按自然日计算、同日=1天；缺证据票排除出平均值并显示样本数，不再要求100%覆盖才显示。',
      readPolicy:'页面读取只读SQLite历史日报/快照/最终结果，不调用CE接口。'
    }
  };
}
...
function register(app){
  ...
  console.info('[CE-QC][V320_DELIVERY_DAILY]',V308_DELIVERY_DAILY_FAST_ID,'daily Shopee/TBKH history is persisted-history-first; average days means real dispatch START→POD sample average, not a coverage percentage gate.');
}
