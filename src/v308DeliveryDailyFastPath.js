...
export const V308_DELIVERY_DAILY_FAST_ID='2026-08-26-v323-single-day-cache-strict-ledger-v2';
...
function strictDailyEvidence(type,date,db){
  ...
  try{return db.prepare(`WITH facts AS (
    ...
    SELECT *,REPLACE(SUBSTR(dispatchAt,1,10),'/','-') dispatchDate,REPLACE(SUBSTR(podDate,1,10),'/','-') podDay
    FROM facts
  )
  SELECT
...
export function readV308DeliveryDaily(businessType='',fromDate='',toDate='',db=getDb()){
  ...
  if(!TYPES.has(type))throw new Error('V323每日派次明细仅支持TBKH、SHOPEECN、SHOPEEVN');
  ...
  const daily=(data.daily||[]).map(row=>({
    ...row,ledgerReady:row.ready!==false,podKnown:n(row.pod),ocKnown:n(row.ocCurrent),attempt1Known:n(row.attempt1),attempt2Known:n(row.attempt2),attempt3Known:n(row.attempt3),signingDaysCount:n(row.dispatchSigningDaysCount),signingDaysSum:n(row.dispatchSigningDaysSum),avgSigningDays:row.avgDispatchSigningDays,avgDispatchSigningDays:row.avgDispatchSigningDays,signingSampleCount:n(row.dispatchSigningDaysCount),evidenceIncomplete:!row.evidenceComplete||!row.attemptEvidenceComplete||!row.signingEvidenceComplete
  }));
  return{...data,id:V308_DELIVERY_DAILY_FAST_ID,truthId:V320_HISTORICAL_DAILY_TRUTH_ID,daily,avgSigningDays:daily.map(r=>r.avgDispatchSigningDays),avgDispatchSigningDays:daily.map(r=>r.avgDispatchSigningDays),attempt1:daily.map(r=>r.attempt1),attempt2:daily.map(r=>r.attempt2),attempt3:daily.map(r=>r.attempt3),attempt1Rate:daily.map(r=>r.pod?pct(r.attempt1,r.pod):null),attempt2Rate:daily.map(r=>r.pod?pct(r.attempt2,r.pod):null),attempt3Rate:daily.map(r=>r.pod?pct(r.attempt3,r.pod):null),attemptCoverageRate:daily.map(r=>r.attemptCoverageRate),signingCoverageRate:daily.map(r=>r.signingCoverageRate),signingSampleCount:daily.map(r=>r.signingSampleCount),evidenceIncomplete:daily.some(r=>r.evidenceIncomplete),source:'V323_EXPLICIT_MULTI_DAY_V320_PERSISTED_HISTORY',definitions:{history:'多日仅在用户明确选择多日范围后读取。',attempts:'1/2/3派只统计已保存真实派次证据。',averageDays:'平均派件→签收天数只使用真实首次派件START和真实POD日期样本，同日=1天。',readPolicy:'页面只读SQLite已保存数据，不调用CE接口。'}};
}
function handler(req,res){try{const started=Date.now(),data=readV308DeliveryDaily(req.query.businessType,req.query.from,req.query.to);res.setHeader('Cache-Control','private,max-age=10');res.setHeader('X-CE-QC-V308',V308_DELIVERY_DAILY_FAST_ID);res.setHeader('X-CE-QC-V320',V320_HISTORICAL_DAILY_TRUTH_ID);res.setHeader('X-CE-QC-V323','SINGLE_DAY_CACHE_STRICT_LEDGER');res.setHeader('Server-Timing',`v323daily;dur=${Date.now()-started}`);return res.json(data);}catch(error){return res.status(400).json({ok:false,id:V308_DELIVERY_DAILY_FAST_ID,error:error?.message||String(error)});}}
...
function register(app){if(!isRealApp(app)||registeredApps.has(app))return false;registeredApps.add(app);previousGet.call(app,'/api/v308/delivery-daily',handler);console.info('[CE-QC][V323_DELIVERY_DAILY]',V308_DELIVERY_DAILY_FAST_ID,'single-day uses cache + exact membership strict ledger; explicit multi-day retains persisted history.');return true;}
express.application.get=function v323DeliveryDailyRoute(pathValue,...handlers){const path=typeof pathValue==='string'?pathValue:'';if(path.startsWith('/')&&path!=='/api/v308/delivery-daily')register(this);return previousGet.call(this,pathValue,...handlers);};
