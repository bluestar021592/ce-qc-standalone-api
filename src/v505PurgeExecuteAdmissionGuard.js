...
export const V505_PURGE_EXECUTE_ADMISSION_ID='2026-09-12-v505-execute-admission-exact-prepare-v8-sidecar-db-first';
  ...
  if(existingExecute){
    ...
    if(exactReceipt){
      if(existingChallenge===challengeId){
        return {ok:true,recovery:true,status:receiptIsFinalized(exactReceipt)?'FINALIZED':'COMMITTED',jobId,challengeId,durableReceipt:true,receiptFinalized:receiptIsFinalized(exactReceipt),sealedDatabasePath:existingSealedDatabasePath};
      }
    ...
    // receipt from an older generation is historical and may yield to a newly
    // completed PREPARE after the old sidecar itself is no longer active.
