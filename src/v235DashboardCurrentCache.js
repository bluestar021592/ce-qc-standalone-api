...
export const V235_DASHBOARD_CURRENT_CACHE_ID = '2026-08-22-v238-terminal-safe-dashboard-cache-v1';
...
export function refreshV235CurrentDashboardCacheDate(reportDate='',options={}){
  ...
  const rows=[...ccslRows(db,batch),...shopeeRows(db,batch)];const present=new Set(rows.map(row=>String(row.businessType||'').toUpperCase()));
