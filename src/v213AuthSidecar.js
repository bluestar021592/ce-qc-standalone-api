...
export const V249_AUTH_PATH_VERSION='2026-08-20-v249-direct-sidecar-auth-v1';
...
function closeAuthDb(){
  authStmt=null;authDbReady=false;
  if(authDb){try{authDb.close();}catch{}authDb=null;}
}
function openAuthDb(){
  ...
  try{
    ...
    const stmt=db.prepare("SELECT id,username,displayName,departmentCompany,email,passwordHash,role,businessScope,enabled,status,expiresAt,mustChangePassword,lockedUntil FROM users WHERE username=? AND status='ACTIVE' LIMIT 1");
    authDb=db;authStmt=stmt;authDbFile=file;authDbOpenMs=Date.now()-started;authDbReady=true;
    return authDb;
...
function readAuthRow(username){
  try{openAuthDb();return authStmt.get(username)||null;}
    ...
    const retryable=/SQLITE_SCHEMA|SQLITE_BUSY|database is locked|database connection is not open/i.test(String(error?.message||error));
...
function warmAuthDb(){
  const started=Date.now();
  try{openAuthDb();console.log(`[CE-QC][V249_AUTH_DB_READY] readOnly=true openMs=${authDbOpenMs} file=${authDbFile}`);return true;}
  catch(error){authDbReady=false;console.error(`[CE-QC][V249_AUTH_DB_WARM_FAILED] ms=${Date.now()-started}`,error?.message||error);return false;}
}
...
function headers(req,res,status=200){
  ...
  const h={'content-type':'application/json; charset=utf-8','cache-control':'no-store','x-ce-qc-auth-sidecar':V213_AUTH_SIDECAR_VERSION,'x-ce-qc-auth-path':V249_AUTH_PATH_VERSION};
...
function json(req,res,status,payload,extra={}){
  ...
  const h={'content-type':'application/json; charset=utf-8','cache-control':'no-store','x-ce-qc-auth-sidecar':V213_AUTH_SIDECAR_VERSION,'x-ce-qc-auth-path':V249_AUTH_PATH_VERSION,...extra};
  if(allowed.origin){h['access-control-allow-origin']=allowed.origin;h['access-control-allow-credentials']='true';h['vary']='Origin';}
...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==='OPTIONS'){const allowed=allowedOrigin(req);if(!allowed.ok)return json(req,res,403,{ok:false,code:'V213_ORIGIN_DENIED'});const h={'access-control-allow-origin':allowed.origin||'*','access-control-allow-credentials':'true','access-control-allow-headers':'Content-Type','access-control-allow-methods':'GET,POST,OPTIONS','cache-control':'no-store','x-ce-qc-auth-path':V249_AUTH_PATH_VERSION};res.writeHead(204,h);return res.end();}
  if(req.method==='GET'&&url.pathname==='/api/v213/auth-ping')return json(req,res,200,{ok:true,version:V213_AUTH_SIDECAR_VERSION,authPathVersion:V249_AUTH_PATH_VERSION,port:PORT,appPort:APP_PORT,pid:process.pid,dbReady:authDbReady,dbOpenMs:authDbOpenMs});
  ...
  const started=Date.now();
  console.log(`[CE-QC][V249_AUTH_LOGIN_START] channel=${channel}`);
  try{
    ...
    console.log(`[CE-QC][V249_AUTH_READ_OK] user=${username} found=${Boolean(row)} ms=${Date.now()-started} dbOpenMs=${authDbOpenMs}`);
    ...
    console.log(`[CE-QC][V249_AUTH_PASSWORD_CHECK] user=${username} matched=${matched} ms=${Date.now()-started}`);
    ...
    console.log(`[CE-QC][V249_AUTH_LOGIN_OK] user=${username} channel=${channel} ms=${Date.now()-started}`);
    return json(req,res,200,{ok:true,user:issued.payload.user,expiresAt:issued.expiresAt,authMode:'V249_DIRECT_AUTH_SIDECAR',handoffToken:issued.token}, {'set-cookie':issued.cookies});
  }catch(error){
    console.error(`[CE-QC][V249_AUTH_ERROR] ms=${Date.now()-started}`,error?.stack||error);
...
server.requestTimeout=10000;server.headersTimeout=11000;server.keepAliveTimeout=1000;
server.listen(PORT,HOST,()=>console.log(`[CE-QC][V213_AUTH_SIDECAR] READY http://${HOST}:${PORT} · main=${APP_PORT} · ${V213_AUTH_SIDECAR_VERSION} · ${V249_AUTH_PATH_VERSION} · persistent read-only auth DB`));
...
export const __test={hostOnly,ipOnly,privateV4,localChannel,cleanUsername,allowedOrigin,readAuthRow,openAuthDb,closeAuthDb};
