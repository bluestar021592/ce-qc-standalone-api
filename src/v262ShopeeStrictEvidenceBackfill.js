...
export const V262_SHOPEE_STRICT_EVIDENCE_ID='2026-08-23-v262-shopee-unknown-attempt-retry-v1';
...
const PERIODIC_MS=Math.max(2*60*60_000,Number(process.env.V262_STRICT_PERIODIC_MS||6*60*60_000));
...
export function v262ShouldRetryStrictRow(row={}){
  return String(row.terminalReason||'').toUpperCase()==='POD'&&['SHOPEECN','SHOPEEVN'].includes(String(row.businessType||'').toUpperCase())&&Number(row.attemptNo||0)===0;
}
...
function unknownCandidates(db){
  ensureV246TrackingSchema(db);const today=cambodiaClock().date,from=addDays(today,-(LOOKBACK_DAYS-1));
  return db.prepare(`SELECT shipmentCode,businessType,firstReportDate,lastImportedDate,podDate,attemptNo,attemptSource,lastCheckedAt,terminalReason
    FROM qc_tracking_ledger WHERE terminalReason='POD' AND businessType IN ('SHOPEECN','SHOPEEVN') AND attemptNo=0 AND firstReportDate BETWEEN ? AND ?
    ORDER BY CASE WHEN lastCheckedAt='' THEN 0 ELSE 1 END,lastCheckedAt,firstReportDate,shipmentCode LIMIT ?`).all(from,today,MAX_PER_RUN).filter(v262ShouldRetryStrictRow);
}
...
export async function runV262ShopeeStrictEvidenceBackfill({client=new CEClient(),reason='AUTO'}={}){
  ...
  running=true;const startedAt=nowIso();try{
    ...
    let localApplied={updated:0,known:0,unknown:0,podDateFilled:0,corrected:0};if(localEvidence.length)localApplied=applyV246StrictAttemptEvidence(localEvidence,{db,reason:`V262:${reason}:LOCAL_STORED_TRACK`});
    const apiEvidence=[];
    ...
    let apiApplied={updated:0,known:0,unknown:0,podDateFilled:0,corrected:0};if(apiEvidence.length)apiApplied=applyV246StrictAttemptEvidence(apiEvidence,{db,reason:`V262:${reason}:CE_TRACK_RETRY_UNKNOWN`});
    const result={ok:true,id:V262_SHOPEE_STRICT_EVIDENCE_ID,reason,startedAt,completedAt:nowIso(),candidates:candidates.length,localKnown,localUpdated:localApplied.updated,apiQueried,apiFailed,apiUpdated:apiApplied.updated,known:localApplied.known+apiApplied.known,unknown:apiApplied.unknown,podDateFilled:localApplied.podDateFilled+apiApplied.podDateFilled};
    console.log('[CE-QC][V262_SHOPEE_STRICT_EVIDENCE]',JSON.stringify(result));return result;
  }catch(error){console.warn('[CE-QC][V262_SHOPEE_STRICT_EVIDENCE] failed:',error?.message||error);return{ok:false,id:V262_SHOPEE_STRICT_EVIDENCE_ID,reason,error:error?.message||String(error)};}finally{running=false;}
}
...
function start(){
  ...
  periodicTimer=setInterval(async()=>{const r=await runV262ShopeeStrictEvidenceBackfill({reason:'PERIODIC_AUTO'});if(r?.skipped&&r.reason==='FOREGROUND_PROCESSING_ACTIVE')retryIfBlocked();},PERIODIC_MS);periodicTimer.unref?.();
  console.log(`[CE-QC][V262_SHOPEE_STRICT_EVIDENCE] ${V262_SHOPEE_STRICT_EVIDENCE_ID} enabled: stored-track first + CE retry for POD attemptNo=0; ${LOOKBACK_DAYS}d lookback, max ${MAX_PER_RUN}/run.`);
}
start();
