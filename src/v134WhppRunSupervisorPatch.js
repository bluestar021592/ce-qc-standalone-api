...
function idleBackendContinuity(reportDate=''){const date=latchContinuityComplete(reportDate);return{ok:Boolean(date),reportDate:date,pollingActive:Boolean(autoResumeTimer),revision:V134_CONTINUITY_IDLE_REVISION};}
...
async function maybeAutoResumeWhpp(reason='backend-watch'){
  ...
  try{
    const currentState=loadWhppState(),currentDate=dateOnly(currentState.reportDate);if(currentDate){const lock=inspectV378WhppCompletionLock(currentDate,currentState);if(lock.locked){latchContinuityComplete(currentDate);return false;}}
    const [ccslModule,shopeeModule,summaryModule,recoveryModule]=await Promise.all([import('./v317CcslIncompleteRecoveryPatch.js'),import('./v311ShopeeIncompleteRecoveryPatch.js'),import('./v132WhppFastIntegrationPatch.js'),import('./v165WhppRunStateRecoveryPatch.js')]);
    const selectedHint=typeof ccslModule.inspectV317ExplicitReportDateHint==='function'?ccslModule.inspectV317ExplicitReportDateHint({maxAgeMs:60_000}):null;
    ...
    const reportDate=dateOnly(ccsl?.reportDate);if(!reportDate||ccsl?.complete!==true)return false;
    ...
    if (persistedCompletion.locked) latchContinuityComplete(reportDate);
    ...
    const key=`${reportDate}:${Number(before?.total||0)}:${Number(before?.finalEvidenceRows||0)}:${beforeStatus}`,stamp=Date.now();if(key===lastAutoResumeKey&&stamp-lastAutoResumeAt<AUTO_RESUME_COOLDOWN_MS)return false;
    ...
    if (afterPersistedCompletion.locked) latchContinuityComplete(reportDate);
    if (afterPersistedCompletion.locked) return false;
    lastAutoResumeKey=key;lastAutoResumeAt=stamp;const started=launchWhpp('resume');console.log('[CE-QC][V357_WHPP_BACKEND_CONTINUITY]',JSON.stringify({revision:BACKEND_CONTINUITY_REVISION,selectedDateRevision:SELECTED_DATE_CONTINUITY_REVISION,completionLockRevision:V378_WHPP_COMPLETION_LOCK_REVISION,continuityIdleRevision:V134_CONTINUITY_IDLE_REVISION,reportDate,reportDateSource:hintedDate?'VISIBLE_BROWSER_STATUS_HINT':'CANONICAL_FALLBACK',hintAgeMs:Number(selectedHint?.ageMs||0),reason,ccslComplete:true,shopeeComplete:true,whppSnapshotStatus:afterStatus||'PENDING',whppTotal:Number(afterRecovery?.total||0),runtime:started}));return true;
  }catch(error){if(error?.code!=='WHPP_ALREADY_FINALIZED')console.warn('[CE-QC][V357_WHPP_BACKEND_CONTINUITY] skipped:',error?.message||error);return false;}finally{autoResumeBusy=false;}
}
function rearmBackendContinuity(reportDate=''){const date=dateOnly(reportDate);continuityCompletedDate='';lastAutoResumeKey='';lastAutoResumeAt=0;ensureBackendContinuityLoop();if(date){const timer=setTimeout(()=>{void maybeAutoResumeWhpp('new-import-event');},1200);timer.unref?.();}return{ok:true,reportDate:date,pollingActive:Boolean(autoResumeTimer),revision:V134_CONTINUITY_IDLE_REVISION};}
...
export function inspectV134WhppRuntime(){const state=loadWhppState();return{patchId:PATCH_ID,backendContinuityRevision:BACKEND_CONTINUITY_REVISION,selectedDateContinuityRevision:SELECTED_DATE_CONTINUITY_REVISION,completionLockRevision:V378_WHPP_COMPLETION_LOCK_REVISION,finalMaterializationRevision:V134_FINAL_MATERIALIZATION_EVENT_REVISION,continuityIdleRevision:V134_CONTINUITY_IDLE_REVISION,requestTimeoutMs:WHPP_REQUEST_TIMEOUT_MS,runtimeActive:Boolean(runtimePromise && runtime.active),runtime:publicRuntime(runtimePromise && runtime.active?runtime:lastRuntime),completionLock:inspectV378WhppCompletionLock(state.reportDate,state),backendContinuity:{enabled:true,pollMs:AUTO_RESUME_POLL_MS,pollingActive:Boolean(autoResumeTimer),cooldownMs:AUTO_RESUME_COOLDOWN_MS,busy:autoResumeBusy,lastAttemptAt:lastAutoResumeAt,lastKey:lastAutoResumeKey,completedDateLatch:continuityCompletedDate}};}
