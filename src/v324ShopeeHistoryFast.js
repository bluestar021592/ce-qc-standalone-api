...
export const V324_SHOPEE_HISTORY_FAST_ID='2026-08-26-v324-shopee-full-uploaded-history-fast-v1';
...
function baseHistory(type,from,to,db,withLedger=true){const group=groupOf(type);const ledgerJoin=withLedger?`LEFT JOIN qc_tracking_ledger l ON l.shipmentCode=m.shipmentCode AND UPPER(TRIM(l.businessType))='${type}'`:'';const ledgerFields=withLedger?`,l.terminalReason,l.attemptNo,l.attemptSource,l.podDate,l.evidenceJson`:`,'' terminalReason,0 attemptNo,'' attemptSource,'' podDate,'{}' evidenceJson`;const sql=`WITH members AS (
  ...
  FROM members m
  LEFT JOIN business_final_rows f ON UPPER(TRIM(f.businessType))='SHOPEE' AND UPPER(TRIM(f.shipmentCode))=m.shipmentCode AND f.reportDate=m.reportDate
  ${ledgerJoin}
), norm AS (
  SELECT *,REPLACE(SUBSTR(dispatchAt,1,10),'/','-') dispatchDate,REPLACE(SUBSTR(podAt,1,10),'/','-') podDay FROM facts
)
SELECT reportDate,COUNT(*) total,SUM(isPod) pod,
  SUM(CASE WHEN isPod=1 AND podDay=reportDate THEN 1 ELSE 0 END) sameDayPod,
  SUM(CASE WHEN isPod=0 AND (UPPER(TRIM(category))='OC' OR UPPER(TRIM(category)) LIKE 'OC%' OR category LIKE '%OC滞留%') THEN 1 ELSE 0 END) ocCurrent,
