...
const PATCH_ID = '2026-08-17-v177-one-business-one-workbook-job-v1';
...
function writeJsonAtomic(file, value) {
  const temp = `${file}.${process.pid}.tmp`; fs.writeFileSync(temp, JSON.stringify(value, null, 2), 'utf8'); fs.renameSync(temp, file);
  try { cacheJobRead(file, fs.statSync(file), value); } catch { jobReadCache.delete(file); }
}
function readJobFile(file) {
  if (!file) return null; let stat;
  try { stat = fs.statSync(file); } catch { jobReadCache.delete(file); return null; }
  const signature = statSignature(stat); const hit = jobReadCache.get(file);
...
function forgetActive(job = {}) {
  if (!job?.payloadKey) return; const slot = activeSlot(job.payloadKey, job.requestedBy || '');
  if (!job.jobId || activeJobs.get(slot) === job.jobId) activeJobs.delete(slot);
}
function jobFilesExist(job = {}) {
  if (!Array.isArray(job.files) || !job.files.length) return false; const exportDir = getRuntimeConfig().exportsDir;
...
function recentJobFiles() {
  const now = Date.now(); if (jobFileIndexCache.files.length && now - jobFileIndexCache.at < JOB_FILE_INDEX_CACHE_MS) return jobFileIndexCache.files;
  let files = [];
  try {
    files = fs.readdirSync(jobsDir(), { withFileTypes: true }).filter(entry => entry.isFile() && entry.name.endsWith('.json')).map(entry => {
      const file = path.join(jobsDir(), entry.name); let mtimeMs = 0; try { mtimeMs = fs.statSync(file).mtimeMs; } catch {} return { file, mtimeMs };
    }).sort((a, b) => b.mtimeMs - a.mtimeMs).slice(0, MAX_JOB_SCAN);
  } catch { files = []; }
  jobFileIndexCache = { at: now, files }; return files;
}
function recentJobs() { const jobs = []; for (const item of recentJobFiles()) { const job = readJobFile(item.file); if (job) jobs.push(job); } return jobs; }
function activeReusableJob(key, requester = '') {
  const id = activeJobs.get(activeSlot(key, requester)); if (!id) return null; const job = readJob(id);
  ...
  if (['QUEUED', 'RUNNING'].includes(String(job.status || ''))) return { job, reused: 'ACTIVE' }; forgetActive(job); return null;
}
function reusableJob(key, requester = '') {
  const active = activeReusableJob(key, requester); if (active) return active; const now = Date.now();
...
function validatePayload(payload, res) {
  ...
  } else if (!/^\d{4}-\d{2}-\d{2}$/.test(payload.date)) { res.status(400).json({ ok: false, error: '请选择有效的基准日期。' }); return false; }
  return true;
}
function enqueueExport(req, res) {
  const payload = normalizePayload(req.body || {}); if (!validatePayload(payload, res)) return;
  const key = payloadKey(payload); const requester = req.user?.username || req.user?.email || ''; const reusable = reusableJob(key, requester);
  if (reusable) {
    const job = reusable.job; const completed = reusable.reused === 'COMPLETED';
  ...
  const file = jobPath(jobId); const now = new Date().toISOString();
  const job = { version: PATCH_ID, exportContractVersion: EXPORT_CONTRACT_VERSION, jobId, payloadKey: key, status: 'QUEUED', progress: 0, message: '完整报表任务已进入后台队列', payload, files: [], createdAt: now, updatedAt: now, requestedBy: requester };
  writeJsonAtomic(file, job); rememberActive(job); jobFileIndexCache = { at: 0, files: [] };
  const child = spawn(process.execPath, ['--max-old-space-size=1536', workerFile, file], { cwd: getRuntimeConfig().projectRoot, env: process.env, detached: true, windowsHide: true, stdio: 'ignore' }); child.unref();
...
function exportStatus(req, res) {
  const job = readJob(req.params.jobId); if (!job) return res.status(404).json({ ok: false, error: '导出任务不存在或已过期。' });
...
express.application.post = function v177AsyncExportRoute(...args) {
  if (args[0] !== PREPARE_PATH || args.length < 2) return originalPost.apply(this, args);
  if (!installed) { installed = true; originalPost.call(this, PREPARE_PATH, enqueueExport); originalGet.call(this, STATUS_PATH, exportStatus); }
  return this;
};
export function inspectV177ExportCaches(){return {activeJobs:activeJobs.size,fileIndexAgeMs:jobFileIndexCache.at?Date.now()-jobFileIndexCache.at:null,fileCount:jobFileIndexCache.files.length,fileIndexTtlMs:JOB_FILE_INDEX_CACHE_MS,jobReadCache:jobReadCache.size,jobReadCacheMax:JOB_READ_CACHE_MAX,exportContractVersion:EXPORT_CONTRACT_VERSION};}
