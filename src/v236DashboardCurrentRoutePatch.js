...
function normalizeRangeMetric(type,state={},reportDate=''){
  ...
  const exactUnresolved=isWhpp?optionalNumber(raw,['unresolved']):null;
  ...
  for(const code of ['PP','PV','UNKNOWN']){
    ...
    const exactRegionUnresolved=isWhpp?optionalNumber(region,['unresolved']):null;
    const regionUnresolved=exactRegionUnresolved===null?Math.max(0,rt-rp-rr-rc):Math.max(0,exactRegionUnresolved);
    const coverageComplete=!isWhpp||code==='UNKNOWN'||regionCoverageComplete;
    ...
    if(isWhpp&&!coverageComplete){for(const key of ['total','pod','returned','cancelled','unresolved','pending1','pending2','pending3','pendingNonContinuous','ocCurrent','oc1','oc2','oc3','cycle2','inboundNoScan','delivery','deliveryStay','provinceOpen','podRate','returnRate','ocRate'])normalized[key]=null;}
    base.regions[code]=normalized;
  }
  return base;
}
function readV419RangeCurrentSummary(fromDate='',toDate=''){
  const from=dateOnly(fromDate),to=dateOnly(toDate);if(!from||!to||from>to)throw new Error('日期范围无效');if(from===to)return readV236CurrentSummary(to);
  const range=loadRangeDashboard(from,to),resolvedFrom=dateOnly(range?.fromDate)||from,resolvedTo=dateOnly(range?.toDate)||to,business={};
  ...
  return{ok:true,readId:V236_DASHBOARD_CURRENT_READ_ID,rangeReadId:V419_GLOBAL_RANGE_CURRENT_SUMMARY_ID,reportDate:resolvedTo,fromDate:resolvedFrom,toDate:resolvedTo,periodStart:resolvedFrom,periodEnd:resolvedTo,snapshotId:'',snapshotIds:{},snapshotStatus:range?.analysisComplete?'COMPLETED':'PARTIAL',business,whpp,readSource:'V419_CANONICAL_PERIOD_DASHBOARD_RANGE',readSources:Object.fromEntries(V236_TYPES.map(type=>[type,'V419_RANGE'])),cacheOnly:false,readyBusinessCount:Object.values(business).filter(item=>item.ready).length,sourceTotal:n(range?.sourceTotal),analyzedTotal:n(range?.analyzedTotal),analysisPending:n(range?.analysisPending),analysisComplete:Boolean(range?.analysisComplete),visibleTruthId:range?.visibleTruthId||'',queryMode:range?.queryMode||''};
}
function currentHandler(req,res){
  try{
    ...
    const singleDayRequest=!from||from===to;
    if(singleDayRequest&&data.whpp?.membershipIncomplete){
      res.setHeader('Cache-Control','no-store');res.setHeader('X-CE-QC-Current-Read',V236_DASHBOARD_CURRENT_READ_ID);
      return res.status(409).json({ok:false,routeId:V236_DASHBOARD_CURRENT_ROUTE_ID,code:data.whpp.errorCode||'WHPP_STANDARD_DAILY_INCOMPLETE',error:`WHPP标准日报成员不完整：日报头${Number(data.whpp.expected||0)}票，成员${Number(data.whpp.actual||0)}票`,reportDate:data.reportDate,expected:Number(data.whpp.expected||0),actual:Number(data.whpp.actual||0)});
    }
    res.setHeader('Cache-Control','no-store');res.setHeader('X-CE-QC-Current-Read',V236_DASHBOARD_CURRENT_READ_ID);res.setHeader('X-CE-QC-Range-Current',data.rangeReadId||'SINGLE_DAY');return res.json(data);
  }catch(error){return res.status(500).json({ok:false,routeId:V236_DASHBOARD_CURRENT_ROUTE_ID,error:error?.message||String(error)});}
}
function statePayload(type,req,res){const data=readV236CurrentSummary(String(req.query.reportDate||'').slice(0,10)),metric=data.business[type];res.setHeader('Cache-Control','no-store');res.setHeader('X-CE-QC-Current-Read',V236_DASHBOARD_CURRENT_READ_ID);return res.json({ok:true,businessType:type,reportDate:data.reportDate,snapshotId:data.snapshotId,snapshotStatus:metric?.ready?'COMPLETED':data.snapshotStatus,state:stateFromV236Metric(type,metric||{},data)});}
function stateHandler(req,res){try{const type=String(req.params.type||req.params.businessType||'').toUpperCase();if(!V236_TYPES.includes(type))return res.status(400).json({ok:false,error:'业务板块无效'});return statePayload(type,req,res);}catch(error){return res.status(500).json({ok:false,routeId:V236_DASHBOARD_CURRENT_ROUTE_ID,error:error?.message||String(error)});}}
function trendHandler(req,res){try{const data=readV237DashboardTrends(String(req.query.businessType||'ALL'),String(req.query.from||''),String(req.query.to||''));res.setHeader('Cache-Control','private,max-age=10');res.setHeader('X-CE-QC-Trend-Read',V237_DASHBOARD_TREND_READ_ID);return res.json(data);}catch(error){return res.status(500).json({ok:false,routeId:V236_DASHBOARD_CURRENT_ROUTE_ID,error:error?.message||String(error)});}}
function compactAggregate(scope,req,res){try{const data=readV236CurrentSummary(String(req.query.reportDate||'').slice(0,10)),state=aggregateV236State(scope,data);res.setHeader('Cache-Control','no-store');res.setHeader('X-CE-QC-Current-Read',V236_DASHBOARD_CURRENT_READ_ID);return res.json({ok:true,state});}catch(error){return res.status(500).json({ok:false,routeId:V236_DASHBOARD_CURRENT_ROUTE_ID,error:error?.message||String(error)});}}
function compactBusiness(req,res,next){if(String(req.query.compact||'')!=='1')return next();const type=String(req.params.businessType||'').toUpperCase();if(!V236_TYPES.includes(type))return next();return statePayload(type,req,res);}
...
express.application.get=function v237DashboardOwnerRoute(pathValue,...handlers){
  ...
  if(path==='/api/v234/current-summary'){console.info('[CE-QC][V237]',V236_DASHBOARD_CURRENT_ROUTE_ID,'owns V234 current-summary + V419 global range current cards');return originalGet.call(this,pathValue,currentHandler);}
  if(path==='/api/v234/business-state/:type'){console.info('[CE-QC][V237]',V236_DASHBOARD_CURRENT_ROUTE_ID,'owns V234 business-state');return originalGet.call(this,pathValue,stateHandler);}
  if(path==='/api/v234/trends'){console.info('[CE-QC][V237]',V236_DASHBOARD_CURRENT_ROUTE_ID,'owns V234 trends with exact completed snapshot truth');return originalGet.call(this,pathValue,trendHandler);}
  if(path==='/api/business-state/:businessType'){console.info('[CE-QC][V237]',V236_DASHBOARD_CURRENT_ROUTE_ID,'adds fast compact business-state pre-handler');return originalGet.call(this,pathValue,compactBusiness,...handlers);}
  if(path==='/api/shopee/state'){console.info('[CE-QC][V237]',V236_DASHBOARD_CURRENT_ROUTE_ID,'adds fast compact SHOPEE aggregate pre-handler');return originalGet.call(this,pathValue,compactShopee,...handlers);}
  if(path==='/api/state'){console.info('[CE-QC][V237]',V236_DASHBOARD_CURRENT_ROUTE_ID,'adds fast compact CCSL aggregate pre-handler');return originalGet.call(this,pathValue,compactCcsl,...handlers);}
