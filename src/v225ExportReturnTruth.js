...
export const V225_EXPORT_RETURN_TRUTH_ID = '2026-08-31-v380-ledger-plus-strict-export-signing-v1';
...
function evidenceDiagnostics(type,rows){if(!STRICT_DELIVERY_TYPES.has(type))return{businessType:type,pod:rows.filter(r=>r.pod).length,attemptMissing:0,signingMissing:0,partial:false};const podRows=rows.filter(r=>r.pod),attemptMissing=podRows.filter(r=>!Number(r.attemptNo||0)).length,signingMissing=podRows.filter(r=>!Number.isFinite(Number(r.signingDays))||Number(r.signingDays)<=0).length;return{businessType:type,pod:podRows.length,attemptMissing,signingMissing,partial:Boolean(attemptMissing||signingMissing)};}
export async function collectV200Rows(type,range,onProgress=()=>{}){
  ...
  // V380: saved qc_tracking_ledger is genuine persisted evidence and must not be
  // discarded during export. Hydrate it before the strict saved-event overlay;
  // V320 then remains the strongest START->POD owner wherever events exist.
  applyV294ExportAttemptSigningTruth(businessType,rows,{db:getDb(),range});
