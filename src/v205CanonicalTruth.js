...
export const V205_CANONICAL_TRUTH_VERSION = '2026-08-18-v205-canonical-archive-union-family-evidence-v2';
...
const RETRY_RE = /待重试|API.*失败|REFRESH_FAILED|RETRY|SCAN_RETRY|TRACK_RETRY/i;
...
function textOf(raw = {}) { return [raw.eventCode,raw.trackingEventCode,raw.statusCode,raw.shipmentStatus,raw.status,raw.statusText,raw.currentState,raw.scanNormalizedState,raw.退回状态,raw.订单取消,raw.trackingEventDescZh,raw.trackingEventDesc,raw.trackingEventDescKm,raw.remark,raw.place,raw.eventShop,raw.locationCode,raw.latestEventDesc,raw.最后节点,raw.primaryCategory,raw.currentMainCategory,raw.主分类,raw.异常分类,raw.QC判断].map(v=>String(v||'').trim()).filter(Boolean).join(' '); }
...
function appendRemark(existing='',addition=''){const a=String(existing||'').trim(),b=String(addition||'').trim();if(!b)return a;if(a.includes(b))return a;return[a,b].filter(Boolean).join('；').slice(0,900);}
function fallbackDailyStatus(code=''){const c=String(code||'').toUpperCase();if(c==='P')return'Pending（日报状态，轨迹证据待补齐）';if(c==='W'||c==='Y')return'派送中（日报状态，轨迹证据待补齐）';if(c==='R')return'退回';return'';}

function parseMembership(rowJson = {}) {
  const parsed=safeJson(rowJson,{});const raw=parsed?.raw&&typeof parsed.raw==='object'?parsed.raw:parsed;
  const map=new Map(Object.entries(raw||{}).map(([k,v])=>[keyOf(k),v]));
...
function minimalRow(type, source={}) {
  ...
  return {
    ...
    currentShop,currentProvince:get(['当前省份','所在省份','currentprovince']),recipient:String(source.recipientNormalized||source.recipientRaw||get(['收件人','收货人','recipient','receiver','consignee'])||''),
    recipientPhone:get(['收件人手机','收件人电话','收货人手机','收货人电话','手机号','receiverphone']),recipientAddress:get(['收件地址','收货地址','详细地址','地址','receiveraddress']),
    deliveryShop,deliveryProvince:get(['派件省份','派送省份','deliveryprovince']),courier:get(['派件快递员','派送快递员','快递员','deliverycourier','courier']),
    ...
    pod:false,returned,cancelled,terminalNormal:returned||cancelled,openUnpod:!returned&&!cancelled,pending,delivering,store:STORE_RE.test(`${currentShop} ${deliveryShop}`),
    ...
    metricEligible:true,sourceOrigin:'DAILY_CANONICAL_ARCHIVE_UNION',membershipSeenCount:0,membershipSnapshotCount:0,evidenceCoverage:'DAILY_ONLY',latestEventTime:'',latestEventDesc:'',dataIntegrityReview:true
  };
}

function mergeMembershipField(row, source) {
  const {parsed,get}=parseMembership(source.rowJson);
  const sourceKey=`${source.reportDate}|${source.batchCreatedAt||''}|${source.batchId||''}|${String(source.rowNumber||0).padStart(8,'0')}`;
  ...
  if(!row._membershipLatestKey||sourceKey>=row._membershipLatestKey){
    ...
    row.statusCode=code||row.statusCode;row.statusDesc=meaningfulDesc(desc)||meaningfulDesc(row.statusDesc)||fallbackDailyStatus(code);row.rawDeliveryTime=get(['派件时间','签收时间','POD时间','deliverytime','podtime','deliveredat'])||row.rawDeliveryTime;
  ...
  row.area=areaOf(row.regionCode,row.recipientProvince);row.store=Boolean(row.store||STORE_RE.test(`${row.currentShop||''} ${row.deliveryShop||''}`));row.metricEligible=true;row.sourceOrigin='DAILY_CANONICAL_ARCHIVE_UNION';
}

function canonicalMembership(db,type,range) {
  if(type==='WHPP')return[];
  try{return db.prepare(`SELECT r.shipmentCode,r.reportDate,r.regionCode,r.recipientRaw,r.recipientNormalized,r.rowNumber,r.rowJson,r.snapshotId,r.batchId,b.createdAt batchCreatedAt,b.status batchStatus
    ...
    WHERE b.status IN ('VALID','SUPERSEDED') AND b.reportDate BETWEEN ? AND ?
...
function mergeCanonicalMembership(baseRows,type,range,db) {
  const source=canonicalMembership(db,type,range);
  if(type!=='WHPP'&&!source.length)return{rows:baseRows,sourceRows:0,uniqueDaily:baseRows.length,recovered:0,supersededRecovered:0};
  const rows=[...baseRows],byBill=new Map(rows.map((row,index)=>[billOf(row.shipmentCode),index]));let recovered=0,supersededRecovered=0;
  for(const item of source){
    const bill=billOf(item.shipmentCode);if(!bill)continue;let index=byBill.get(bill);
    if(index===undefined){const row=minimalRow(type,item);rows.push(row);index=rows.length-1;byBill.set(bill,index);recovered++;if(item.batchStatus==='SUPERSEDED')supersededRecovered++;}
    mergeMembershipField(rows[index],item);
  }
  ...
  return{rows,sourceRows:source.length,uniqueDaily:new Set(source.map(r=>billOf(r.shipmentCode)).filter(Boolean)).size,recovered,supersededRecovered};
}

function classifyEvidence(raw={},code='') {
  const c=String(code||raw.eventCode||raw.trackingEventCode||raw.statusCode||raw.shipmentStatus||'').trim().toUpperCase();const current=String(raw.currentState||raw.scanNormalizedState||'').trim().toUpperCase();const text=textOf(raw);
  if(current==='POD'||c==='80'||c==='4004'||String(raw.orderStatus||'')==='85'||POD_RE.test(text))return{kind:'POD',label:'POD',terminal:true,normal:true};
  if(['RETURNED','RETURN_COMPLETED'].includes(current)||c==='86'||c==='P4008'||String(raw.orderStatus||'')==='100'||RETURN_RE.test(text))return{kind:'RETURNED',label:'退回',terminal:true,normal:true};
  if(current==='ORDER_CANCELLED'||CANCEL_RE.test(text)||String(raw.orderStatus||'')==='10')return{kind:'CANCELLED',label:'取消订单',terminal:true,normal:true};
  if(current==='RETURN_IN_PROGRESS'||c==='84'||/退回处理中|RETURN_IN_PROGRESS/i.test(text))return{kind:'RETURN_IN_PROGRESS',label:'退回处理中',terminal:false,normal:true};
  ...
  if(RETRY_RE.test(text))return{kind:'RETRY',label:'接口待重试',terminal:false,normal:false};
  if(c==='150'||current==='PENDING'||PENDING_RE.test(text))return{kind:'PENDING',label:'Pending',terminal:false,normal:false};
  if(c==='70'||c==='4003'||current==='DELIVERING'||DELIVERY_RE.test(text))return{kind:'DELIVERING',label:'派送中',terminal:false,normal:true};
  if(c==='60'||current==='ASSIGNED'||ASSIGN_RE.test(text))return{kind:'ASSIGNED',label:'分配派送中',terminal:false,normal:true};
  ...
  if(OC_RE.test(text))return{kind:'OC',label:'OC',terminal:false,normal:false};
  if(/入库无扫描/i.test(text))return{kind:'INBOUND_NO_SCAN',label:'入库无扫描',terminal:false,normal:false};
  ...
  if(text)return{kind:'TRACK_NODE',label:'轨迹有节点',terminal:false,normal:true};
...
function applyTruth(row,raw={},source='EVIDENCE',forceTerminal=false){
  const code=String(raw.eventCode||raw.trackingEventCode||raw.lastEventCode||raw.latestTrackStatusCode||raw.shipmentStatus||raw.statusCode||'').trim();const t=truthTime(raw),text=textOf(raw),cls=classifyEvidence(raw,code);if(cls.kind==='UNKNOWN')return false;
  if(!shouldReplaceTruth(row,t,forceTerminal||cls.terminal&&['85','100','10'].includes(String(raw.orderStatus||''))))return false;
  row.latestEventTime=t||row.latestEventTime||'';row.latestEventDesc=String(raw.latestEventDesc||raw.最后节点||raw.lastEventDesc||text||'').trim();row.evidenceCoverage=source;row.dataIntegrityReview=cls.kind==='RETRY';
  ...
  row.terminalNormal=row.pod||row.returned||row.cancelled;row.openUnpod=!row.terminalNormal;row.store=cls.kind==='STORE'||Boolean(row.store&&cls.kind!=='DELIVERING');row.statusDesc=cls.label||meaningfulDesc(row.statusDesc)||fallbackDailyStatus(row.statusCode);
  if(!cls.normal&&!cls.terminal&&cls.label)row.exceptionDesc=cls.label;
  return true;
}
function applyStateRow(row,stateRow={}){
  if(!allowedEvidenceType(row.businessType,stateRow.businessType))return false;
  const state=String(stateRow.state||'').toUpperCase(),api=String(stateRow.apiStatus||'').toUpperCase();
  if((state==='PENDING_SCAN'||api==='PENDING_SCAN')&&!stateRow.lastEventTime)return false;
  const raw={...safeJson(stateRow.stateJson,{}),currentState:stateRow.state,latestEventTime:stateRow.lastEventTime,apiStatus:stateRow.apiStatus,updatedAt:stateRow.updatedAt};
  const current=String(raw.currentState||raw.scanNormalizedState||'').toUpperCase();const terminalLock=['POD','RETURNED','RETURN_COMPLETED','CANCELLED','ORDER_CANCELLED'].includes(current)||['85','100','10'].includes(String(raw.orderStatus||''));
  return applyTruth(row,raw,'CURRENT_STATE',terminalLock);
}
...
function enrichFamilyEvidence(rows,type,db){
  const byBill=new Map(rows.map(row=>[billOf(row.shipmentCode),row])),bills=[...byBill.keys()].filter(Boolean);if(!bills.length)return{track:0,current:0,final:0,noEvidence:0};let trackCount=0,currentCount=0,finalCount=0;
  ...
  for(const row of rows){
    if(row.terminalNormal){row.pending=false;row.delivering=false;row.statusDesc=row.pod?'POD':row.returned?'退回':'取消订单';row.dataIntegrityReview=false;}
      ...
      else if(row.statusCode==='R'){row.returned=true;row.terminalNormal=true;row.openUnpod=false;row.statusDesc='退回';row.dataIntegrityReview=false;}
      else if(MANUAL_REVIEW_RE.test(row.statusDesc||''))row.statusDesc='轨迹证据待补齐';
      noEvidence++;row.dataIntegrityReview=true;
    } else if(MANUAL_REVIEW_RE.test(row.statusDesc||'')) row.statusDesc=row.latestEventDesc?'轨迹有节点':'轨迹证据待补齐';
...
export async function collectV205CanonicalRows(type,range,onProgress=()=>{}){
  ...
  let base=[];let baseError='';try{base=await collectV200Rows(businessType,range,onProgress);}catch(error){baseError=error?.message||String(error);if(businessType==='WHPP')throw error;}
  const merged=mergeCanonicalMembership(base,businessType,range,db);if(!merged.rows.length&&baseError)throw new Error(baseError);
  const evidence=enrichFamilyEvidence(merged.rows,businessType,db);
  const official=merged.rows.filter(row=>row.metricEligible!==false),uniqueBills=new Set(official.map(row=>billOf(row.shipmentCode)).filter(Boolean));
  if(businessType!=='WHPP'&&merged.uniqueDaily&&uniqueBills.size!==merged.uniqueDaily)throw new Error(`V205完整性对账失败：${businessType} 历史日报唯一票 ${merged.uniqueDaily}，导出候选 ${uniqueBills.size}`);
  onProgress({phase:'v205CanonicalTruth',completed:merged.rows.length,total:merged.rows.length,businessType,canonicalSourceRows:merged.sourceRows,canonicalUniqueDaily:merged.uniqueDaily,recoveredFromOlderCompletedSnapshots:merged.recovered,recoveredFromSupersededSnapshots:merged.supersededRecovered,trackTruth:evidence.track,currentTruth:evidence.current,finalTruth:evidence.final,noEvidence:evidence.noEvidence,baseWarning:baseError,version:V205_CANONICAL_TRUTH_VERSION});
