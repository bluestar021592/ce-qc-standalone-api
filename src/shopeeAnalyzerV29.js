...
const OUTBOUND_RE = /(^|\s)outbound(\s|$)|出库|离开网点|货物离开|发往|转往|下一个网点/i;
const INBOUND_RE = /pickup\s*inbound|(^|\s)inbound(\s|$)|入库|到仓|货物到达网点|到达门店/i;
const CYCLE_COUNT_RE = /cycle\s*count|盘点/i;
const TRANSIT_HUB_RE = /(?:CE|CEL)\s*:\s*(?:WHJT|WHPP)\b|\b(?:WHJT|WHPP)\b/i;

export const SHOPEE_ANALYSIS_RULE_VERSION = '2026-08-09-latest-effective-node-v29';
...
export function analyzeShopeeShipment({
  ...
  // IMPORTANT: business counters below are CURRENT-state counters. Historical
  // Pending/OC evidence is preserved separately, but a later effective node closes
  // the previous Pending/OC episode so old evidence cannot keep a parcel abnormal.
  const pending = analyzePendingCycles(sorted, isPod, isReturned);
  ...
  // A parcel can be old but still have a fresh node today. Severe overdue is based
  // on NO NEW NODE, never on age since the original report/import date.
  const severeOverdue = Boolean(!isPod && !isReturned && staleDays >= 3);
  ...
  const pvOpenDisposition = classifyPvOpenDisposition({
    region, storeFlow, deliveryDays, staleDays, isPod, isReturned, apiFailed
  });
  ...
  else if (storeFlow.shopState === 'SHOP_ARRIVED_CURRENT') category = Number(storeFlow.shopRetentionNaturalDays || 0) >= 2 ? '门店滞留' : '门店入库';
  else if (storeFlow.shopState === 'SHOP_TRANSFER_IN_PROGRESS') category = staleDays >= 2 ? '门店途中2天+' : '门店途中';
  else if (noTrack) category = '无轨迹';
  else if (severeOverdue) category = '严重超时未更新';
  else if (atTransitHub) category = staleDays >= 2 ? '中转节点停留' : '中转节点流转';
  ...
  const currentPendingDays = pending.activeDays;
  ...
  return {
    ...
    pendingContinuity: pending.activeDays >= 2 ? (pending.activeContinuous ? '连续' : '不连续') : (pending.activeDays ? '单日' : '无'),
    ...
    中转节点停留: atTransitHub && staleDays >= 2 ? '是' : '否',
...
function classifyPvOpenDisposition({ region, storeFlow, deliveryDays, staleDays, isPod, isReturned, apiFailed }) {
  ...
  if (storeFlow?.shopState === 'SHOP_TRANSFER_IN_PROGRESS' || Number(deliveryDays || 0) > 0) {
    return Number(staleDays || 0) >= 2 ? 'PV_DELIVERY_STALE' : 'PV_DELIVERY_IN_PROGRESS';
  }
  return Number(staleDays || 0) >= 2 ? 'PV_OTHER_UNRESOLVED' : 'PV_OTHER_PROGRESS';
}

function pvOpenDispositionLabel(value) {
  return ({
    PV_DELIVERY_IN_PROGRESS: '外省派送/转运中',
    PV_DELIVERY_STALE: '外省派送/转运2天+未更新',
    PV_STORE_NORMAL: '外省门店正常流转',
    PV_STORE_RETENTION: '外省门店滞留',
    PV_OTHER_PROGRESS: '外省其他正常流转',
    PV_OTHER_UNRESOLVED: '外省其他2天+未更新'
  })[value] || '';
}
...
function analyzePendingCycles(events, isPod, isReturned) {
  const cycles = [];
  let activeDates = [];
  let thresholdAt = '';
  ...
  for (const event of events) {
    ...
    if (isPendingEvent(event)) {
      ...
      if (!thresholdOpen && longestConsecutive(activeDates) >= 3) {
        thresholdOpen = true;
        thresholdAt = date;
      }
      continue;
    ...
    if (isEffectiveProgressEvent(event)) {
      if (activeDates.length) cycles.push(activeDates);
      if (thresholdOpen) {
        latestActionAfterThreshold = (isDeliveryAssignEvent(event) || isDeliveryEvent(event) || isOutboundEvent(event)) ? 'delivery' : 'progress';
      }
      activeDates = [];
    }
  }
  ...
  const returnRequired = Boolean(thresholdOpen && !isPod && !isReturned);
  return {
    cycles,
    allDates,
...
function analyzeOc(items, events, reportDate, isPod, isReturned) {
  ...
  if (isPod) return { active: false, days: 0, items: ocItems, startTime: '', closeReason: 'POD_CLOSED' };
  if (isReturned) return { active: false, days: 0, items: ocItems, startTime: '', closeReason: 'RETURN_CLOSED' };

  // An OC episode is active only when the OC record is newer than the latest
  // effective business progress node. Any later Pending/inbound/outbound/shop/
  // delivery/cycle/POD/return node means the previous OC is no longer current.
  const lastProgress = findLatest(events, isEffectiveProgressEvent);
  const progressTime = String(lastProgress?.eventTime || '');
  const activeItems = progressTime ? ocItems.filter(item => itemTime(item) > progressTime) : ocItems;
  if (!activeItems.length) {
    return { active: false, days: 0, items: ocItems, startTime: '', closeReason: 'STATE_CHANGED_BY_NEW_NODE' };
  }
  const startTime = itemTime(activeItems[0]);
  return { active: true, days: elapsedInclusiveDays(startTime, reportDate), items: activeItems, startTime, closeReason: '' };
}
...
function buildTags({ pending, oc, cycleCount, inboundNoScan, deliveryDays, staleDays, noTrack, isReturned, returnPhoto, apiFailed, region, atTransitHub, severeOverdue }) {
  const tags = [];
  const pendingDays = pending.activeDays;
...
function itemTime(item = {}) { return String(item.reportTime || item.lastUpdateDate || item.creationDate || ''); }
...
function isEffectiveProgressEvent(event = {}) {
  return isPodEvent(event) || isReturnEvent(event) || isPendingEvent(event) || isDeliveryAssignEvent(event)
    || isDeliveryEvent(event) || isOutboundEvent(event) || isInboundEvent(event) || isCycleCountEvent(event);
}
...
function eventText(event = {}) {
  return [event.eventCode, event.trackingEventCode, event.trackingEventDesc, event.trackingEventDescZh, event.trackingEventDescKm, event.place, event.locationCode, event.eventShop, event.remark]
    .map(value => String(value || '')).join(' ');
}
