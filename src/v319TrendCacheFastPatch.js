...
export const V319_TREND_CACHE_FAST_ID='2026-08-26-v324-auto-full-uploaded-shopee-trend-v1';
...
const SHOPEE_SET=new Set(SHOPEE_TYPES);
...
export function readV319TrendCacheFast(businessType='ALL',fromDate='',toDate='',db=getDb(),options={}){
  ...
  if(options.historyAll&&SHOPEE_SET.has(type)){const data=readV308DeliveryDaily(type,from,to,db,{historyAll:true});return trendShape({...data,historyExpanded:true,source:`V324_AUTO_FULL_UPLOADED_HISTORY:${data.source||''}`},type,from,to);}
  ...
  return trendShape({...data,source:`V323_EXPLICIT_RANGE_HISTORY:${data.source||V320_HISTORICAL_DAILY_TRUTH_ID}`},type,from,to);
}
function handler(req,res){try{const started=Date.now(),historyAll=String(req.query.history||'').toLowerCase()==='all',data=readV319TrendCacheFast(req.query.businessType,req.query.from,req.query.to,getDb(),{historyAll});res.setHeader('Cache-Control','private,max-age=10');res.setHeader('X-CE-QC-V319',V319_TREND_CACHE_FAST_ID);res.setHeader('X-CE-QC-V320',V320_HISTORICAL_DAILY_TRUTH_ID);res.setHeader('X-CE-QC-V324',historyAll?'AUTO_FULL_UPLOADED_HISTORY':'EXACT_RANGE');res.setHeader('Server-Timing',`v324trend;dur=${Date.now()-started}`);return res.json(data);}catch(error){return res.status(400).json({ok:false,id:V319_TREND_CACHE_FAST_ID,error:error?.message||String(error)});}}
...
function register(app){if(!isRealApp(app)||registeredApps.has(app))return false;registeredApps.add(app);previousGet.call(app,'/api/v319/trends',handler);console.info('[CE-QC][V324_TREND_ROUTE]',V319_TREND_CACHE_FAST_ID,'Shopee single-day pages may auto-load full persisted uploaded history via V324 fast reader; current cards remain exact-day cache-only.');return true;}
express.application.get=function v324TrendAvailabilityRoute(pathValue,...handlers){const path=typeof pathValue==='string'?pathValue:'';if(path.startsWith('/')&&path!=='/api/v319/trends')register(this);return previousGet.call(this,pathValue,...handlers);};
