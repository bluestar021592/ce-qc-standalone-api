...
export const V319_TREND_CACHE_FAST_ID='2026-08-26-v321-single-day-cache-multiday-history-v3';
...
export function readV319TrendCacheFast(businessType='ALL',fromDate='',toDate='',db=getDb()){
  ...
  return{...data,id:V319_TREND_CACHE_FAST_ID,readId:V319_TREND_CACHE_FAST_ID,source:`V321_EXPLICIT_RANGE_HISTORY:${data.source||V320_HISTORICAL_DAILY_TRUTH_ID}`};
}
function handler(req,res){try{const started=Date.now(),data=readV319TrendCacheFast(req.query.businessType,req.query.from,req.query.to);res.setHeader('Cache-Control','private,max-age=10');res.setHeader('X-CE-QC-V319',V319_TREND_CACHE_FAST_ID);res.setHeader('X-CE-QC-V320',V320_HISTORICAL_DAILY_TRUTH_ID);res.setHeader('X-CE-QC-V321','SINGLE_DAY_CACHE_MULTIDAY_EXPLICIT_HISTORY');res.setHeader('Server-Timing',`v321;dur=${Date.now()-started}`);return res.json(data);}catch(error){return res.status(400).json({ok:false,id:V319_TREND_CACHE_FAST_ID,error:error?.message||String(error)});}}
function register(app){if(registered)return;registered=true;previousGet.call(app,'/api/v319/trends',handler);console.info('[CE-QC][V321_TREND_AVAILABILITY_FIRST]',V319_TREND_CACHE_FAST_ID,'single-day first paint is dashboard-cache-only; multi-day history runs only after an explicit multi-day selection; startup auto-backfill is not loaded.');}
