...
export const V319_TREND_CACHE_FAST_ID='2026-08-26-v328-three-business-auto-history-trend-v1';
...
export function readV319TrendCacheFast(businessType='ALL',fromDate='',toDate='',db=getDb(),options={}){
  ...
  if(options.historyAll&&ATTEMPT_SET.has(type)){const data=readV308DeliveryDaily(type,from,to,db,{historyAll:true});return trendShape({...data,historyExpanded:true,source:`V328_AUTO_THREE_BUSINESS_HISTORY:${data.source||''}`},type,from,to);}
  ...
  return trendShape({...data,source:`V328_EXPLICIT_RANGE_HISTORY:${data.source||V320_HISTORICAL_DAILY_TRUTH_ID}`},type,from,to);
}
function handler(req,res){try{const started=Date.now(),historyAll=String(req.query.history||'').toLowerCase()==='all',data=readV319TrendCacheFast(req.query.businessType,req.query.from,req.query.to,getDb(),{historyAll});res.setHeader('Cache-Control','private,max-age=10');res.setHeader('X-CE-QC-V319',V319_TREND_CACHE_FAST_ID);res.setHeader('X-CE-QC-V320',V320_HISTORICAL_DAILY_TRUTH_ID);res.setHeader('X-CE-QC-V328',historyAll?'THREE_BUSINESS_AUTO_HISTORY':'EXACT_RANGE');res.setHeader('Server-Timing',`v328trend;dur=${Date.now()-started}`);return res.json(data);}catch(error){return res.status(400).json({ok:false,id:V319_TREND_CACHE_FAST_ID,error:error?.message||String(error)});}}
...
function register(app){if(!isRealApp(app)||registeredApps.has(app))return false;registeredApps.add(app);previousGet.call(app,'/api/v319/trends',handler);console.info('[CE-QC][V328_TREND_ROUTE]',V319_TREND_CACHE_FAST_ID,'TBKH + SHOPEE CN/VN single-day pages auto-load their own persisted history through the shared V308 reader; current cards remain exact-day cache-only.');return true;}
express.application.get=function v328TrendAvailabilityRoute(pathValue,...handlers){const path=typeof pathValue==='string'?pathValue:'';if(path.startsWith('/')&&path!=='/api/v319/trends')register(this);return previousGet.call(this,pathValue,...handlers);};
