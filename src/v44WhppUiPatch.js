...
// V468: V464 backend remains absent from the eager startup import chain; V462 lazy-dispatches its exact route only.

const PATCH_ID='2026-09-13-v509-no-live-trend-shell-v1';
const V554_INTERACTION_READY_ID='2026-09-18-v554-nonblocking-legacy-body-loader-v1';
const V555_CRITICAL_SHELL_INLINE_ID='2026-09-19-v555-critical-shell-inline-v1';
// Compatibility-only source signature for the historical V433 go-live gate; V509 remains the active shell owner: const PATCH_ID='2026-09-05-v433-unified-status-stability-loader-v1'
...
function safeInlineAssetPath(assetPath=''){
  const relative=String(assetPath||'').replace(/^\/+/, '');
  const resolved=path.resolve(PUBLIC_DIR,relative);
  const root=PUBLIC_DIR.endsWith(path.sep)?PUBLIC_DIR:PUBLIC_DIR+path.sep;
  if(!resolved.startsWith(root))throw new Error('CRITICAL_SHELL_ASSET_OUTSIDE_PUBLIC');
  return resolved;
}

function inlineCriticalShellAssets(html=''){
  const readAsset=assetPath=>fs.readFileSync(safeInlineAssetPath(assetPath),'utf8');
  let output=String(html||'');
  output=output.replace(/<link\s+([^>]*?)href="\/([^"?]+\.css)(?:\?[^"]*)?"([^>]*)>/gi,(full,before,assetPath,after)=>{
    const attrs=`${before||''}${after||''}`;
    if(!/\brel\s*=\s*["']stylesheet["']/i.test(attrs))return full;
    const css=readAsset(assetPath).replace(/<\/style/gi,'<\\/style');
    return `<style data-ce-qc-inline-src="/${assetPath}">\n${css}\n</style>`;
  });
  output=output.replace(/<script\s+([^>]*?)src="\/([^"?]+\.js)(?:\?[^"]*)?"([^>]*)><\/script>/gi,(full,before,assetPath,after)=>{
    const js=readAsset(assetPath).replace(/<\/script/gi,'<\\/script');
    const attrs=`${before||''}${after||''}`.trim();
    return `<script${attrs?` ${attrs}`:''} data-ce-qc-inline-src="/${assetPath}">\n${js}\n</script>`;
  });
  return output;
}
...
function html(req,res,next){
  ...
  try{
    ...
    res.setHeader('X-CE-QC-V554-Interaction-Ready',V554_INTERACTION_READY_ID);
    res.setHeader('X-CE-QC-V555-Critical-Shell-Inline',V555_CRITICAL_SHELL_INLINE_ID);
...
export const V555_CRITICAL_SHELL_INLINE_PATCH_ID=V555_CRITICAL_SHELL_INLINE_ID;