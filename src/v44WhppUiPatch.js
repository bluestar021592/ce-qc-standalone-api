...
function buildInjectedHtml(){
  ...
  // V554: compatibility scripts after the base app must never hold the HTML parser.
  // Defer preserves their declared order and DOMContentLoaded behavior while the
  // already-rendered base shell keeps working if an optional legacy asset is slow.
  const nonBlockingBodyInjection=bodyInjection.replace(/<script src=/g,'<script defer src=');
