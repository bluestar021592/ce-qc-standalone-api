...
export const V294_POST_PROCESS_ATTEMPT_BACKFILL_ID = '2026-08-25-v294-post-process-attempt-backfill-v1';
...
function runBackfill(reportDate) {
  ...
  setImmediate(() => {
    try {
      ...
      console.info('[CE-QC][V294_POST_PROCESS_ATTEMPT_BACKFILL_DONE]', JSON.stringify(result));
...
console.info('[CE-QC][V294_POST_PROCESS_ATTEMPT_BACKFILL]', V294_POST_PROCESS_ATTEMPT_BACKFILL_ID,
  'successful CCSL/SHOPEE processing schedules saved-evidence strict attempt reconciliation for TBKH + SHOPEE CN/VN; failed runs never publish attempt changes.');
