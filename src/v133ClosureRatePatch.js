...
const PATCH_ID = '2026-08-14-v133-unified-closure-rate-v1';
...
function rate(closed, total) { return total ? Number((Number(closed || 0) * 100 / Number(total)).toFixed(2)) : 0; }

function latestBatch(db, requestedDate = '') {
  const date = dateOnly(requestedDate);
  if (date) {
    return db.prepare(`SELECT batchId,snapshotId,reportDate,createdAt FROM unified_import_batches WHERE status='VALID' AND reportDate=? ORDER BY createdAt DESC LIMIT 1`).get(date) || null;
  }
...
function hasCompletedSnapshot(db, businessType, reportDate, total) {
  ...
  return Boolean(db.prepare(`
    SELECT 1 FROM business_export_snapshots
...
function coreRows(db, batch) {
  ...
  const rows = db.prepare(`
    ...
    FROM (
      SELECT DISTINCT businessType,shipmentCode
      FROM unified_import_rows
      WHERE snapshotId=?
    ) u
    LEFT JOIN carryover_open_items c
      ON c.shipmentCode=u.shipmentCode AND c.businessType=u.businessType
...
function whppRow(db, reportDate) {
  const row = db.prepare(`
    ...
    LEFT JOIN carryover_open_items c
      ON c.shipmentCode=w.shipmentCode AND c.businessType='WHPP'
  `).get(reportDate) || {};
...
function buildSummary(requestedDate = '') {
  ...
  if (!batch) return { ok:true, patchId:PATCH_ID, reportDate:'', total:0, closed:0, unresolved:0, closureRate:0, completed:true, byBusiness:{} };
  const key = `${batch.snapshotId}|${batch.reportDate}`;
  const hit = cache.get(key);
  if (hit && Date.now() - hit.at < CACHE_MS) return { ...hit.payload, cacheHit:true };

  const core = coreRows(db, batch);
  core.set('WHPP', whppRow(db, batch.reportDate));
  const byBusiness = {};
  let total = 0;
  let unresolved = 0;
  ...
  for (const type of TYPES) {
    ...
    const safeUnresolved = Math.max(0, Math.min(Number(row.total || 0), Number(row.unresolved || 0)));
    const closed = Math.max(0, Number(row.total || 0) - safeUnresolved);
    const completed = hasCompletedSnapshot(db, type, batch.reportDate, row.total);
    byBusiness[type] = {
      total:Number(row.total || 0),
      closed,
      unresolved:safeUnresolved,
      closureRate:completed ? rate(closed, row.total) : null,
      completed
    };
    total += Number(row.total || 0);
    unresolved += safeUnresolved;
    if (Number(row.total || 0) > 0 && !completed) allCompleted = false;
  }
  ...
  const payload = {
    ...
    closed,
    unresolved,
    closureRate:allCompleted ? rate(closed, total) : null,
...
function handler(req, res) {
  try {
    const payload = buildSummary(req.query.reportDate || req.query.date || '');
    res.setHeader('Cache-Control', 'private, max-age=10');
    res.setHeader('Server-Timing', 'v133;desc=closure-summary');
    res.json(payload);
  } catch (error) {
...
let installed = false;
express.application.listen = function v133ClosureListen(...args) {
  if (!installed) {
    installed = true;
