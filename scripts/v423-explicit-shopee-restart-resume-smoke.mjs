...
assert.match(source,/V424_RESUME_FLOOR_HANDOFF_REVISION='2026-09-04-v424-v169-v67-proof-handoff-v1'/);
...
async function runCase(lastTruth,{rejectHandoff=false}={}){
  const calls={start:0,resume:0,handoff:null,factory:0};
  const document={
    readyState:'loading',
  ...
  const window={
    ...
    resumeUnified:async handoff=>{calls.resume+=1;calls.handoff=handoff;return{ok:true,owner:'V67',mode:'resume',handoff};},
    __CE_QC_V168_SEVEN_BUSINESS_STATUS__:{lastTruth,refresh:async()=>lastTruth},
    __CE_QC_V67_RESILIENT_RUN_GUARD__:{
      createShopeeRestartHandoff(input){
        calls.factory+=1;
        if(rejectHandoff)return null;
        return{revision:'2026-09-04-v424-restart-proof-resume-floor-v1',resumeStage:'SHOPEE',reportDate:input.reportDate,checkedAt:input.checkedAt,testToken:true};
      }
    }
  };
...

{
  ...
  assert.equal(calls.factory,1,'exact restart must ask V67 to certify the same-proof resume floor');
  assert.equal(calls.resume,1,'exact current-date restart interruption must delegate once to V67 resume');
  assert.equal(calls.handoff?.resumeStage,'SHOPEE','V169 must carry a SHOPEE resume-floor handoff, not a bare resume');
  assert.equal(result.mode,'resume');
  assert.equal(api.v423Revision,'2026-09-04-v423-explicit-shopee-restart-resume-v1');
  assert.equal(api.v424Revision,'2026-09-04-v424-v169-v67-proof-handoff-v1');
}
{
  const {calls,result}=await runCase(truth(),{rejectHandoff:true});
  assert.equal(calls.start,0,'rejected same-proof handoff must fail closed instead of reopening CCSL');
  assert.equal(calls.resume,0,'rejected handoff must not call bare V67 resume');
  assert.equal(result.code,'SHOPEE_RESTART_HANDOFF_REJECTED');
}
{
...
console.log('[V424/V423] explicit SHOPEE restart handoff smoke passed · exact current-date PROCESS_RESTART_INTERRUPTED after CCSL completion requires V67 same-proof resume-floor token · rejected handoff fails closed · generic failure/wrong-date/incomplete-CCSL stay on normal V67 start · V169 owns no processing API');