...
const file=path.join(os.tmpdir(),`ce-qc-v202-${process.pid}.xlsx`);
try{
  ...
  must(names.join('|')==='每日看板|全部明细|金边明细|外省明细|门店明细|POD明细|未POD明细|分配派送中明细|Pending明细|退回明细','reference workbook must keep exact 10-sheet structure');
  ...
  must(stats.overall.days[0]===2,'average signing days must use order date to actual POD date');
  console.log('[V202] generated reference workbook structure/style/link/average smoke passed');
