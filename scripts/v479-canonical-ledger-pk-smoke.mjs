...
import { applyV419CanonicalExportLedgerTruth, V419_CANONICAL_EXPORT_LEDGER_TRUTH_ID, V479_CANONICAL_LEDGER_READ_ID, V493_SCAN85_POD_DATE_RECOVERY_ID, V495_SAVED_TERMINAL_EVENT_POD_DATE_ID } from '../src/v419CanonicalExportLedgerTruth.js';
...
assert.match(v419,/2026-09-10-v495-shipment-current-state-terminal-event-pod-date-v1/);
...
assert.match(v419,/strictMissingPodDateBills/,'V493/V495 may read saved local state only for strict POD members whose canonical podDate is blank');
assert.match(v419,/shipmentCurrentStateByBill/,'V495 must have a second local-only fallback to shipment_current_state for blank historical ledger JSON');
assert.match(v419,/SELECT shipmentCode,state,apiStatus,lastEventCode,lastEventTime,stateJson FROM shipment_current_state/,'V495 local fallback must read saved terminal proof plus lastEventTime');
assert.doesNotMatch(v419,/SELECT[^`]*updatedAt[^`]*FROM shipment_current_state/,'V495 must never read shipment_current_state.updatedAt as POD evidence');
...
assert.doesNotMatch(v419,/lastCheckedAt\].*scan85SavedPodDate|scan85SavedPodDate[\s\S]{0,400}lastCheckedAt/,'V493/V495 must never fabricate POD date from export/ledger check time');
...
assert.equal(V495_SAVED_TERMINAL_EVENT_POD_DATE_ID,'2026-09-10-v495-shipment-current-state-terminal-event-pod-date-v1');
...
try{
  ...
  db.exec(`CREATE TABLE shipment_current_state (
    ...
    lastEventCode TEXT NOT NULL DEFAULT '',
  ...
  insert.run('SPE-V493-1','SHOPEECN','2026-07-10','2026-09-01','','','TERMINAL','POD',now,'POD','POD','2026-07-13 18:00:00','',2,'V246_STRICT_TRACK:TEST',null,
    ...
    '{}',now,'',now,now);
  db.prepare(`INSERT INTO shipment_current_state(shipmentCode,businessType,reportDate,state,apiStatus,lastEventCode,lastEventTime,stateJson,updatedAt) VALUES(?,?,?,?,?,?,?,?,?)`)
    .run('SPE-V493-1','SHOPEECN','2026-07-10','POD','success','85','2026-07-13 18:00:00',JSON.stringify({orderStatus:'85',updateTime:'2026-07-13 16:45:00',currentState:'POD',POD时间:''}),now);
  ...
  assert.equal(shopeeRows[0].podDate,'2026-07-13','saved terminal lastEventTime must recover the real POD date without CE re-query');
  ...
  assert.equal(shopeeRows[0].podSource,'V495_SAVED_TERMINAL_EVENT_TIME');
  assert.equal(shopeeRows[0].v495SavedTerminalEventPodDateId,V495_SAVED_TERMINAL_EVENT_POD_DATE_ID);
  ...
  assert.ok(shopeeSql.some(sql=>/SELECT shipmentCode,state,apiStatus,lastEventCode,lastEventTime,stateJson FROM shipment_current_state/.test(sql)),'blank ledger JSON must fall back to saved terminal state locally');
  ...
  assert.equal(shopeeDiag.savedTerminalEventPodDateRows,1);
  assert.equal(shopeeDiag.terminalStateFallbackRows,1);
  assert.equal(shopeeDiag.strictEvidenceRows,1);
}finally{db.close();}

console.log('[V496/V495/V493/V479] canonical V246 export ledger smoke passed · shipmentCode PK SEARCH · strict blank-POD rows recover saved terminal lastEventTime locally · no updatedAt/export-time fabrication · no CE re-query · strict START→POD signing completes');
