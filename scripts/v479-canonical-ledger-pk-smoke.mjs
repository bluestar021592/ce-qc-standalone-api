...
assert.match(v419,/2026-09-09-v493-scan85-saved-terminal-pod-date-v1/);
...
assert.equal(V493_SCAN85_POD_DATE_RECOVERY_ID,'2026-09-09-v493-scan85-saved-terminal-pod-date-v1');
...
try{
  ...
  insert.run('SPE-V493-1','SHOPEECN','2026-07-10','2026-09-01','','','TERMINAL','POD',now,'POD','POD','2026-07-13 18:00:00','',2,'V246_STRICT_TRACK:TEST',null,
    ...
    JSON.stringify({orderStatus:'85',updateTime:'2026-07-13 16:45:00',currentState:'POD',POD时间:''}),now,'',now,now);
  ...
  assert.ok(shopeeSql.some(sql=>/SELECT shipmentCode,currentStateJson AS jsonValue/.test(sql)),'strict POD with blank canonical podDate must read saved currentStateJson separately');
...
console.log('[V493/V479] canonical V246 export ledger smoke passed · shipmentCode PK SEARCH · OPEN JSON stays selective · strict blank-POD rows recover saved scan85 updateTime locally · no CE re-query · strict START→POD signing completes');
