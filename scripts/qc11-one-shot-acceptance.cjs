...
// Golden product shell: direct 5177 internal login, no later sidecar/handoff chain.
...
// Product completeness inherited from the Aug-17 shell.
...
// WHPP and seven-business parity must remain first class.
...
// Ensure the updater executes this final gate rather than the abandoned auth-sidecar gate.
...
// Deterministic business/export smoke checks.
for(const script of [
...
// Real HTTP process test against an isolated temporary SQLite DB.
