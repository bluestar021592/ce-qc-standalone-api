...
assert.match(progressUi,/2026-09-05-v427-v168-idle-status-owner-v1/,'V138 must expose the V427 V168-idle/V138-live owner contract');
assert.match(progressUi,/2026-09-05-v427-v168-idle-status-single-owner-v1/,'V138 must declare V168 as the single idle exact-date status owner');
...
assert.match(progressUi,/function activeV67Ccsl\(\)[\s\S]*stage\?\.owner==='V67'[\s\S]*stage\.active===true[\s\S]*===\s*'CCSL'/,'V138 live detail must be restricted to an actually active V67 CCSL stage');
assert.match(progressUi,/function v168OwnsIdleLegacyStatus\(node\)[\s\S]*activeV67Ccsl\(\)[\s\S]*__CE_QC_V168_SEVEN_BUSINESS_STATUS__/,'V168 must own idle detail on the modern shell');
assert.match(progressUi,/postJson\('\/api\/v317\/ccsl-recovery',[\s\S]*action:'status',[\s\S]*reportDate:reportDate\|\|''/,'V317 selected-date recovery remains available only as a legacy-shell compatibility fallback');
assert.match(progressUi,/if\(global\.__CE_QC_V168_SEVEN_BUSINESS_STATUS__\)\{\s*if\(!activeV67Ccsl\(\)\)return null;[\s\S]*else\{[\s\S]*canonicalCcsl\(reportDate\)/,'modern V168 shell must return before idle V317 recovery while legacy shells may retain V317 fallback');
assert.match(progressUi,/\/api\/v33\/run-progress\?businessType=\$\{encodeURIComponent\(normalizedType\)\}/,'active V67 CCSL must read V33 live progress');
assert.match(progressUi,/truth\.complete===true\|\|truth\.zeroTicketDay===true/,'legacy V317 fallback must still close canonical completed/zero-ticket dates without requiring a run lock');
assert.match(progressUi,/truth\.dailyExists===false[\s\S]*noDaily:true/,'legacy fallback selected date with no CCSL daily truth must show no-daily/needs-no-work rather than pending 0\/0');
assert.match(progressUi,/canonicalTruth:true,[\s\S]*complete:true/,'legacy canonical completion must still synthesize a completed detail state');
assert.match(progressUi,/当日有效日报CCSL为0票，无需启动订单扫描或轨迹查询/,'0-ticket lower panel must visibly close as completed when legacy fallback is used');
assert.match(progressUi,/所选日期没有CCSL有效日报\/票据，不创建订单扫描或轨迹查询任务/,'no CCSL daily must never be represented as a pending scan run');
assert.match(progressUi,/if\(polling\|\|!page\|\|page\.hidden\|\|v168OwnsIdleLegacyStatus\(status\)\)return/,'idle V138 tick must stop before status I/O while V168 owns the modern page');
assert.match(progressUi,/function enforceLastTruth\(\)[\s\S]*if\(v168OwnsIdleLegacyStatus\(status\)\)return/,'legacy 250ms repaint enforcement must be inert while V168 owns idle truth');
assert.match(progressUi,/status&&\(unifiedOwnsLegacyStatus\(status\)\|\|v168OwnsIdleLegacyStatus\(status\)\)/,'V138 renderer must never repaint over V168 idle fail-closed truth');
assert.doesNotMatch(progressUi,/location\.pathname!=='\/import'/,'SPA URL text must not disable CCSL detail polling');
assert.match(progressUi,/setInterval\(enforceLastTruth,250\)/,'active-run CCSL live detail reinforcement remains available');
...
assert.match(htmlOwner,/v138-ccsl-scan-progress\.js\?v=20260905-v427-1/,'browser must load the V427 V168-idle/V138-live CCSL 350/50 detail owner');
