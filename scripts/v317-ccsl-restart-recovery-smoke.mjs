...
assert.match(sevenStatus,/STATUS_SOURCE_REVISION = '2026-09-02-v322-one-read-seven-business-status-v1'/,'V168 must bind to the exact V322 persisted status contract');
...
assert.match(sevenStatus,/stageFromPersisted\(raw\.CCSL, 'CCSL',[\s\S]*stageFromPersisted\(raw\.SHOPEE, 'SHOPEE',[\s\S]*stageFromPersisted\(raw\.WHPP, 'WHPP'/,'one V322 payload must produce all three exact-date stage badges');
...
assert.match(runner,/2026-09-02-v67-persisted-three-stage-runner-v2/,'V67 must expose the current consolidated three-stage runner');
assert.match(runner,/2026-08-29-single-unified-runner-v1/,'V67 is the sole three-stage execution owner');
assert.match(runner,/STATUS_SOURCE_REVISION = '2026-09-02-v322-one-read-seven-business-status-v1'/,'V67 execution decisions must share the same exact persisted status truth as V168');
...
assert.match(runner,/PROCESS_RESTART_INTERRUPTED/,'only the exact persisted process-restart marker may trigger automatic Shopee recovery');
...
assert.match(runner,/recoverPendingWhpp/,'V67 must own automatic continuation to pending WHPP');
...
assert.match(htmlOwner,/2026-09-02-consolidated-persisted-status-loader-v1/,'HTML owner must expose the consolidated V322 persisted-status loader');
...
assert.match(htmlOwner,/PERSISTED_STATUS_BUILD='2026-09-02-v322-one-read-seven-business-status-v1'/,'HTML response must expose the persisted status contract');
...
assert.match(htmlOwner,/v67-resilient-run-guard\.js\?v=20260902-persisted-status-2/,'browser must force-load the consolidated V67 runner');
...
assert.match(htmlOwner,/v168-seven-business-status\.js\?v=20260902-persisted-status-1/,'browser must force-load the one-read persisted V168 status owner');
...
console.log('[V322/V411/V410/V378.1/V378/V377/SINGLE-RUNNER/V341/V334/V317] smoke passed · pending selected date beats stale committed input · one exact-date persisted status read supplies CCSL/SHOPEE/WHPP · exact-date last-good progress survives transient fetch failures · unconfirmed truth blocks duplicate start/resume · V67 remains sole execution owner · exact restart interruption retries after bounded cooldown · scan=350 · trajectory=50');
