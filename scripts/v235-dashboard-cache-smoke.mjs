...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ce-qc-v238-'));
process.env.DATA_DIR = tempRoot;
process.env.DB_FILE = path.join(tempRoot, 'v238-smoke.db');
...
const snapshotId = 'V238-SNAPSHOT-1';
const batchId = 'V238-BATCH-1';
...
db.prepare('INSERT INTO unified_import_batches(batchId,snapshotId,reportDate,sourceName,fileHash,status,summaryJson,warningsJson,createdAt) VALUES(?,?,?,?,?,?,?,?,?)')
  .run(batchId,snapshotId,date,'v238-smoke.xlsx','hash-v238','VALID','{}','[]',now);
...
const insertCcsl = db.prepare(`INSERT INTO final_rows(shipmentCode,reportDate,isPod,category,primaryCategory,pendingDays,ocDays,cycleCountDays,deliveringDays,rawJson,createdAt,updatedAt)
  VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`);
insertCcsl.run('C1',date,1,'POD闭环','POD闭环',5,4,3,2,JSON.stringify({'Pending连续性':'不连续'}),now,now);
...
const insertShopee = db.prepare(`INSERT INTO business_final_rows(businessType,shipmentCode,reportDate,isPod,primaryCategory,rawJson,createdAt,updatedAt,podAttemptNo)
  VALUES('SHOPEE',?,?,?,?,?,?,?,?)`);
insertShopee.run('N1',date,1,'POD',JSON.stringify({podAttemptNo:1,'Pending次数':3,'OC天数':3,'盘点天数':3,'派送中停留天数':2}),now,now,1);
...
insertShopee.run('V1',date,1,'POD',JSON.stringify({podAttemptNo:2,'Pending次数':4,'OC天数':4}),now,now,2);
insertShopee.run('V2',date,0,'派送中停留',JSON.stringify({'Pending次数':0,'OC天数':1,'派送中停留天数':1}),now,now,0);
...
assert.deepEqual({total:metrics('SHOPEECN').total,pod:metrics('SHOPEECN').pod,returned:metrics('SHOPEECN').returned,pending1:metrics('SHOPEECN').pending1,oc1:metrics('SHOPEECN').oc1},{total:3,pod:1,returned:1,pending1:1,oc1:0});
assert.deepEqual({total:metrics('SHOPEEVN').total,pod:metrics('SHOPEEVN').pod,pending1:metrics('SHOPEEVN').pending1,oc1:metrics('SHOPEEVN').oc1},{total:2,pod:1,pending1:0,oc1:1});

const trendAfter = readV237DashboardTrends('SHOPEECN',date,date);
assert.deepEqual(trendAfter.missingDates,[]);
assert.deepEqual({date:trendAfter.daily[0].reportDate,total:trendAfter.daily[0].total,pod:trendAfter.daily[0].pod,returned:trendAfter.daily[0].returned,pending1:trendAfter.daily[0].pending1},{date,total:3,pod:1,returned:1,pending1:1});
assert.equal(trendAfter.ticket[0],3);
assert.equal(trendAfter.podRate[0],33.33);
assert.equal(trendAfter.returnRate[0],33.33);
...
console.log(`[V238] terminal-safe direct current + nested-1203 + PP/PV + cache-only trend smoke passed · ${V235_DASHBOARD_CURRENT_CACHE_ID}`);
