...
assert.equal(Number(pod.signingDays),3,'signing days must be immutable first-report date through actual POD date');
...
const correctedRow=db.prepare("SELECT attemptNo,attemptSource,signingDays FROM qc_tracking_ledger WHERE shipmentCode='TBKH-TRUE-POD'").get();
...
assert.equal(Number(correctedRow.signingDays),3);
