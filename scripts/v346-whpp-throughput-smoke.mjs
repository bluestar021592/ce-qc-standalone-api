...

{
  ...
  const raw = { async confirmQuery(codes) { calls.push([...codes]); active += 1; maxActive = Math.max(maxActive, active); assert.ok(codes.length <= 350); await new Promise(r => setTimeout(r, 12)); active -= 1; return codes.map(shipmentCode => ({ shipmentCode, orderStatus: 70 })); }};
  ...
  assert.equal(client.__ceQcThroughputPools.confirm.planMode, 'compact-pending');
...

{
  ...
  const raw = { async confirmQuery(codes) { calls.push([...codes]); await new Promise(r => setTimeout(r, 3)); return codes.map(shipmentCode => ({ shipmentCode, orderStatus: 70 })); }};
  const state = { businessType: 'WHPP', scanPool: bills, scanQueryStatus: done.map(shipmentCode => ({ shipmentCode, status: 'success' })) };
  const client = createUnifiedThroughputClient(state, raw, { businessType: 'WHPP', confirmConcurrency: 1, hardBudgetMs: 1000 });
  const expected = splitFixedBatches(pending, 350);
  for (const batch of expected) await client.confirmQuery(batch);
  assert.deepEqual(calls.map(batch => batch.join('|')).sort(), expected.map(batch => batch.join('|')).sort());
  assert.equal(new Set(calls.flat()).size, pending.length, 'WHPP scan resume must not duplicate or requery completed tickets');
...

{
  ...
  let active = 0, maxActive = 0; const calls = [];
  const raw = { async trackQuery(codes) { calls.push([...codes]); active += 1; maxActive = Math.max(maxActive, active); assert.ok(codes.length <= 50); await new Promise(r => setTimeout(r, 25)); active -= 1; return codes.map(shipmentCode => ({ shipmentCode })); }};
  ...
  const expected = splitFixedBatches(pending, 50), started = performance.now();
...

{
  ...
  let active = 0, maxActive = 0; const calls = [];
  const raw = { async exceptionQuery(codes) { calls.push([...codes]); active += 1; maxActive = Math.max(maxActive, active); assert.ok(codes.length <= 50); await new Promise(r => setTimeout(r, 20)); active -= 1; return codes.map(shipmentCode => ({ shipmentCode })); }};
...
// Fixed trajectory retry unit: a failed 50-ticket request is not recursively split.
{
  ...
  const raw = { async trackQuery(codes) { sizes.push(codes.length); active += 1; maxActive = Math.max(maxActive, active); await new Promise(r => setTimeout(r, 8)); active -= 1; if (!failedParent && codes.length === 50 && codes[0] === bills[0]) { failedParent = true; throw new Error('synthetic WHPP parent failure'); } return codes.map(shipmentCode => ({ shipmentCode })); }};
