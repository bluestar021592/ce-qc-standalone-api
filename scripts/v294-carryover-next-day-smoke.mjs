...
try{
  ...
  assert.deepEqual(open.map(row=>row.shipmentCode),['TBKH-OPEN'],'only OPEN carry is eligible for explicit manual unfinished-POD refresh');

  const rolloverDate=new Date('2026-08-25T00:05:00+07:00');
  assert.equal(dueCarryRefreshReason(db,rolloverDate),'','Cambodia day rollover must not start automatic carry refresh in manual-only release mode');
  ...
  assert.equal(dueCarryRefreshReason(db,new Date('2026-08-25T03:05:00+07:00')),'','two-hour polling must remain disabled; unfinished POD updates only on explicit manual request');
  ...
  assert.equal(db.prepare(`SELECT status FROM carryover_open_items WHERE shipmentCode='CE-NORMAL-TRANSIT'`).get().status,'OPEN','normal routing/transit must continue next-day tracking when an explicit manual refresh is requested');
  ...
  assert.match(activation,/startCarryoverRefreshScheduler\(\)/,'formal listen patch may invoke the scheduler owner after listening so its manual-only state is explicit');
  assert.match(activation,/server\.once\('listening', activate\)/,'scheduler owner must not initialize until the HTTP server is actually listening');
  assert.match(v147,/v294CarryoverSchedulerActivation\.js/,'startup chain must install carry scheduler ownership before server routes run');
  ...
  assert.match(scheduler,/export function dueCarryRefreshReason\(\) \{ return ''; \}/,'automatic Cambodia rollover/two-hour due reasons must remain disabled');
  assert.match(scheduler,/automatic refresh disabled; unfinished POD data updates only by explicit manual request/,'scheduler owner must advertise the manual-only contract');
  assert.match(scheduler,/started: false, reason: 'MANUAL_ONLY'/,'background scheduler must fail closed into MANUAL_ONLY mode');
  assert.match(scheduler,/V294_KEEP_OPEN_NORMAL_TRANSIT/,'manual carry refresh must normalize normal routing nodes so legacy NORMAL_FINAL closure cannot remove it from tracking');
  ...
  console.log('[V294] carryover next-day smoke passed · OPEN survives next day · explicit manual refresh remains bounded · automatic 00:05/2h refresh stays disabled · WHPP POD locks stay terminal without rescanning');
}finally{db.close();}
