...
try{
  ...
  await waitFor(()=>cdp.eval("(()=>{const links=[...document.querySelectorAll('.side-nav .side-link')];return links.length>=14&&links.every(n=>n.tagName==='A'&&!!n.getAttribute('href'));})()",1500),4000,80,'stable native sidebar normalization');
  ...
  assert.ok(first.links.every(x=>x.tag==='A'&&x.href),'every sidebar route must settle to a native anchor');
