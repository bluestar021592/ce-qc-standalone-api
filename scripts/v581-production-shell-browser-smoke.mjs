...
async function waitFor(fn,timeout=20000,interval=100){const end=Date.now()+timeout;while(Date.now()<end){try{const v=await fn();if(v)return v}catch{}await new Promise(r=>setTimeout(r,interval));}throw new Error('timeout waiting for condition');}
...
try{
  ...
  const target=await waitFor(async()=>{const r=await fetch('http://127.0.0.1:'+debugPort+'/json');if(!r.ok)return null;const list=await r.json();return list.find(x=>x.type==='page'&&x.webSocketDebuggerUrl)||null;},15000,100);
  ...
  await cdp.eval("(()=>{const a=document.querySelector('.app-body'),t=document.querySelector('.topbar'),m=document.querySelector('.main-content'),h=document.getElementById('homePage');a.style.setProperty('display','none','important');t.style.setProperty('display','none','important');m.style.setProperty('display','none','important');h.hidden=true;h.style.setProperty('display','none','important');window.__CE_QC_V581_STABLE_SHELL__.enforce('production-browser-forced-blank');return true;})()");
  await waitFor(()=>cdp.eval("(()=>{const a=getComputedStyle(document.querySelector('.app-body')),t=getComputedStyle(document.querySelector('.topbar')),m=getComputedStyle(document.querySelector('.main-content')),h=document.getElementById('homePage');return a.display!=='none'&&t.display!=='none'&&m.display!=='none'&&!h.hidden&&getComputedStyle(h).display!=='none';})()"),4000,80);

  await click(cdp,'.side-nav a[data-page="ce"]');
  await waitFor(()=>cdp.eval("location.pathname==='/ce'&&!!window.__CE_QC_V581_STABLE_SHELL__&&document.readyState==='complete'"),12000,100);
  await waitFor(()=>cdp.eval("(()=>{const p=document.getElementById('ccslPage'),t=document.getElementById('pageTitle');return p&&!p.hidden&&getComputedStyle(p).display!=='none'&&t?.textContent==='CE看板';})()"),8000,100);

  await click(cdp,'.side-nav a[data-page="import"]');
  await waitFor(()=>cdp.eval("location.pathname==='/import'&&!!window.__CE_QC_V581_STABLE_SHELL__&&document.readyState==='complete'"),12000,100);
  await waitFor(()=>cdp.eval("(()=>{const p=document.getElementById('importPage'),t=document.getElementById('pageTitle');return p&&!p.hidden&&getComputedStyle(p).display!=='none'&&t?.textContent==='数据导入';})()"),8000,100);

  const delivered=await new Promise((resolve,reject)=>{
    const req=http.request({host:'127.0.0.1',port,path:'/?auth=v581',headers:{Cookie:'ce_qc_local_auth_v431='+signedCookie(secret)}},res=>{const chunks=[];res.on('data',c=>chunks.push(c));res.on('end',()=>resolve(Buffer.concat(chunks).toString('utf8')));});req.on('error',reject);req.end();
  });
