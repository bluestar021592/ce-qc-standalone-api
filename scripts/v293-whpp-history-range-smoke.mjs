...
history.run('WHPP','2026-08-24',JSON.stringify({
  reportDate:'2026-08-24',total:9,pod:8,sameDayPod:7,ocCurrent:6,pendingNonContinuous:5,pending3:4,oc1:3,oc2:2,cycle2:2,
  shopRetention2:2,workOrder:2,inboundNoScan:2,provinceOpen:2,returned:2,attempt1:5,attempt2:2,attempt3:1,attemptUnknown:1,
  signingDaysSum:33,signingDaysCount:8
}));
...
for(const key of ['pod','sameDayPod','ocCurrent','pendingNonContinuous','pending3','oc1','oc2','cycle2','shopRetention2','workOrder','inboundNoScan','provinceOpen','returned','attempt1','attempt2','attempt3','attemptUnknown','signingDaysSum','signingDaysCount']){
...
db.close();
console.log('[V293] WHPP history range smoke passed · exact current vs fully rotated vs partial-damage membership separated · denominator-mismatched history publishes total only and zero stale metrics · 236/235 rejected · 0/0 retained · SQLite unchanged');
