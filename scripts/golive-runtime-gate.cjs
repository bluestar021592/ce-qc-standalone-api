...
must(shell, '2026-08-18-v193-isolated-export-owner-shell-v1');
must(shell, '/v108-route-lazy-features.js?v=20260818-v193-1');
...
must(v140, '&compact=1');
must(v108, '2026-08-18-v193-route-lazy-isolated-export-v1');
must(v108, '/v190-export-direct-route-client.js?v=20260818-v191-1');
must(v108, '/v84-async-export-ui.js?v=20260818-v193-1');
...
must(asyncExportUi, '2026-08-18-v193-isolated-export-sidecar-v1');
must(asyncExportUi, 'ce_qc_active_export_job_v193');
must(asyncExportUi, "LEGACY_JOB_KEYS = ['ce_qc_active_export_job_v180','ce_qc_active_export_job_v186','ce_qc_active_export_job_v187','ce_qc_active_export_job_v191','ce_qc_active_export_job_v192']");
must(asyncExportUi, 'claimExclusiveDom');
must(asyncExportUi, 'exportProgressV193');
must(asyncExportUi, 'exportGeneratedFilesV193');
must(asyncExportUi, '[V193独立导出]');
must(asyncExportUi, 'http://${location.hostname}:5178');
must(asyncExportUi, '/api/v193/export-ping');
must(asyncExportUi, '/api/v193/export-period/prepare');
must(asyncExportUi, 'retirePreviousController');
...
must(bootstrap, 'V193 isolated export sidecar starting');
...
must(exportSidecar, '2026-08-18-v193-isolated-export-sidecar-v1');
must(exportSidecar, '/api/v193/export-ping');
must(exportSidecar, '/api/v193/export-period/prepare');
must(exportSidecar, '/api/v193/export-job/:jobId');
must(exportSidecar, 'V193_ISOLATED_SINGLE_BUSINESS');
must(exportSidecar, '[CE-QC][V193_EXPORT_SIDECAR] READY');
...
console.log('[GOLIVE] runtime-source gate passed; V193 starts an isolated port-5178 export service outside the busy dashboard process, V193 UI routes single-business prepare/status directly to that sidecar, and the V191 cross-day truth worker remains the Excel source of truth');
