...
must(shell, '/v108-route-lazy-features.js?v=20260818-v192-1');
must(shell, '/v190-export-direct-route-client.js?v=20260818-v191-1');
must(shell, '/v84-async-export-ui.js?v=20260818-v192-1');
...
must(v140, '&compact=1');
must(v108, '2026-08-18-v192-route-lazy-unblocked-export-v1');
must(v108, '/v190-export-direct-route-client.js?v=20260818-v191-1');
must(v108, '/v84-async-export-ui.js?v=20260818-v192-1');
...
must(asyncExportUi, '2026-08-18-v192-export-ui-unblocked-direct-v1');
must(asyncExportUi, 'ce_qc_active_export_job_v192');
must(asyncExportUi, "LEGACY_JOB_KEYS = ['ce_qc_active_export_job_v180','ce_qc_active_export_job_v186','ce_qc_active_export_job_v187','ce_qc_active_export_job_v191']");
must(asyncExportUi, 'claimExclusiveDom');
must(asyncExportUi, 'exportProgressV192');
must(asyncExportUi, 'exportGeneratedFilesV192');
must(asyncExportUi, 'ceQcRetiredExportUi');
must(asyncExportUi, '[V192直连进度]');
must(asyncExportUi, "const prepareUrl = singleBusiness ? '/api/v190/export-period/prepare' : '/api/export-period/prepare';");
must(asyncExportUi, "singleBusiness ? 'V192导出创建接口' : '七业务导出创建接口'");
...
console.log('[GOLIVE] runtime-source gate passed; V192 manual history summary prevents main-thread export starvation, cache-busted V192 export UI owns the button/progress, and V192 early direct route install guarantees single-business prepare/status endpoints before static app routes; V191 cross-day truth worker verified');
