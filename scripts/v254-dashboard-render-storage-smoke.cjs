...
for(const file of ['public/dashboard-v18.js','public/dashboard-chart-v18.js','src/v89StaticAssetCachePatch.js','src/v262ShopeeStrictEvidenceBackfill.js','src/v263DeliveryKpiTrendPatch.js','scripts/v262-shopee-strict-evidence-smoke.mjs','scripts/v263-delivery-kpi-trend-smoke.mjs'])execFileSync(process.execPath,['--check',file],{stdio:'pipe'});
...
assert.match(inject,/legacy visual overlays retired/,'UI delivery must explicitly retire historical visual owners');
for(const old of ['v237-home-dashboard-owner.js','v244-shopee-trend-owner.js','v250-shopee-metric-visibility.js','v252-qc-lifecycle-ui.js','v254-dashboard-render-rescue.js','v261-dashboard-final-owner.js'])assert.ok(inject.includes(`'${old}'`),`V263 must strip old overlay ${old}`);
...
assert.match(strictBackfill,/businessType IN \('TBKH','SHOPEECN','SHOPEEVN'\)/,'unknown POD retry query must include TBKH and both Shopee boards');
assert.match(strictBackfill,/business_track_events/,'stored trajectory must be used before CE retry');
assert.match(strictBackfill,/CE_TRACK_RETRY_UNKNOWN/,'remaining unknown POD evidence must retry CE trajectory');
...
console.log('[V263] canonical DashboardV18 + cache reset + exact three-business attempt/signing tracking + storage safety gate passed');
