...
let db=getDb();const now='2026-07-14T20:00:00Z';
function seed(date,bill,events){
  ...
  db.prepare(`INSERT INTO unified_import_rows(batchId,snapshotId,reportDate,businessType,shipmentCode,regionCode,recipientRaw,recipientNormalized,sheetName,rowNumber,classificationReason,rowJson,createdAt) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(batch,snap,date,'SHOPEECN',bill,'PP','SHOPEECN','SHOPEECN','日报',1,'V334','{}',now);
  db.prepare(`INSERT INTO business_final_rows(businessType,shipmentCode,reportDate,isPod,primaryCategory,recipient_group,rawJson,createdAt,updatedAt) VALUES(?,?,?,?,?,?,?,?,?)`).run('SHOPEE',bill,date,1,'POD','CN','{}',now,now);
...
closeDb();
execFileSync(process.execPath,['scripts/v329-three-business-cache-worker.mjs','--type=SHOPEECN','--to=2026-07-14'],{cwd:process.cwd(),env,stdio:'pipe',timeout:30000});
db=getDb();
...
assert.equal(rows.length,2);assert.deepEqual(rows.map(r=>r.reportDate),['2026-07-13','2026-07-14']);
...
const {readV329ThreeBusinessDailyCache}=await import('../src/v329ThreeBusinessDailyCache.js');const history=readV329ThreeBusinessDailyCache('SHOPEECN','2026-07-14',db);assert.equal(history.daily[0].firstAttemptRate,100);assert.equal(history.daily[1].firstAttemptRate,0);assert.equal(history.daily[0].firstAttemptEvidenceComplete,true);assert.equal(history.daily[1].firstAttemptEvidenceComplete,true);assert.equal(history.daily[0].avgSigningDays,1);assert.equal(history.daily[1].avgSigningDays,1);
closeDb();fs.rmSync(root,{recursive:true,force:true});
console.log('[V334] historical first-attempt worker runtime smoke passed · saved 70→80 = first-attempt success · saved 70→150→70→80 = second-attempt POD · no production DB and no CE network required');
