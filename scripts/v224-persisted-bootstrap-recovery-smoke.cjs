...
const must = (source, token) => {
  if (!source.includes(token)) throw new Error(`V227 recovery gate missing: ${token}`);
};
const run = args => {
  const result = spawnSync(process.execPath, args, { encoding: 'utf8', env: process.env, timeout: 180000 });
  if (result.error) throw result.error;
  if (result.status !== 0) throw new Error(`V227 command failed: node ${args.join(' ')}\n${result.stdout || ''}\n${result.stderr || ''}`);
...
console.log('[V227] candidate runtime gate passed: loopback health + latest report date + persisted bootstrap + auth handoff + session + 7 business non-zero + WHPP + guarded shell.');
