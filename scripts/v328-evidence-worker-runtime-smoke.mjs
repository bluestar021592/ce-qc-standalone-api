...
closeDb();
for(const [type,to] of [['TBKH','2026-07-01'],['SHOPEECN','2026-07-13']])execFileSync(process.execPath,['scripts/v328-three-business-evidence-worker-v2.mjs',`--type=${type}`,`--to=${to}`],{cwd:process.cwd(),env,stdio:'pipe',timeout:30000});
db=getDb();
for(const [type,date] of [['TBKH','2026-07-01'],['SHOPEECN','2026-07-13']]){const row=db.prepare(`SELECT * FROM v328_attempt_daily_cache WHERE businessType=? AND reportDate=?`).get(type,date);assert.ok(row,`${type} worker cache missing`);assert.equal(Number(row.total),1);assert.equal(Number(row.pod),1);assert.equal(Number(row.attempt1),1);assert.equal(Number(row.signingDaysCount),1);assert.equal(Number(row.signingDaysSum),1);const ledger=db.prepare(`SELECT podDate,attemptNo,attemptSource FROM qc_tracking_ledger WHERE businessType=? AND shipmentCode=?`).get(type,type==='TBKH'?'TBKH-W1':'CN-W1');assert.equal(ledger.podDate,date);assert.equal(Number(ledger.attemptNo),1);assert.match(String(ledger.attemptSource),/严格|V246_STRICT_TRACK|轨迹70/);}
closeDb();fs.rmSync(root,{recursive:true,force:true});
console.log('[V328.3] isolated evidence worker runtime smoke passed · TBKH + SHOPEE CN saved 70→80 events produce attempt1 + real same-day signing=1 without CE network');
