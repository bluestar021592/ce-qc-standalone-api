...
function writeDailyCache(db,baseline,evidenceByKey,source='V328_V2_SAVED_ONLY_STRICT_TRACK'){ensureDailyCache(db);const byDay=new Map();for(const [key,e] of evidenceByKey){const d=key.slice(0,10);if(!byDay.has(d))byDay.set(d,{attempt1:0,attempt2:0,attempt3:0,sum:0,count:0});const x=byDay.get(d),a=Number(e.attemptNo||0);if(a===1)x.attempt1++;else if(a===2)x.attempt2++;else if(a>=3)x.attempt3++;const days=inclusiveDays(e.start,e.podDate);if(days>0){x.sum+=days;x.count++;}}
...
async function main(){
  ...
  const rows=[...candidates.values()],evidenceByKey=new Map(),needSaved=[];send({status:'RUNNING',phase:'SAVED_EVIDENCE',fromDate:from,toDate:to,total:rows.length,completed:0,queried:0,networkRepairEnabled:ALLOW_NETWORK_REPAIR,message:`${type} 历史派次/签收仅从已保存SQLite证据建立缓存`});
  ...
  const network=await optionalNetworkRepair(db,still,evidenceByKey);
  if(!ALLOW_NETWORK_REPAIR&&still.length)send({status:'RUNNING',phase:'SAVED_EVIDENCE_ONLY',total:still.length,completed:still.length,queried:0,failed:0,known:evidenceByKey.size,message:`${type} 已保存证据读取完成；${still.length}票缺少历史START/POD证据，保持未知，不自动请求CE接口`});
  writeDailyCache(db,baseline,evidenceByKey,ALLOW_NETWORK_REPAIR?'V328_V2_SAVED_PLUS_EXPLICIT_NETWORK_STRICT_TRACK':'V328_V2_SAVED_ONLY_STRICT_TRACK');const expectedPod=(baseline.daily||[]).reduce((s,r)=>s+Number(r.pod||0),0),known=evidenceByKey.size,unresolved=Math.max(0,expectedPod-known);send({status:'COMPLETED',phase:'DONE',fromDate:from,toDate:to,total:expectedPod,completed:expectedPod,queried:network.queried,failed:network.failed,known,unresolved,networkKnown:network.networkKnown,networkRepairEnabled:ALLOW_NETWORK_REPAIR,message:`${type} 历史缓存完成；已保存START+POD证据 ${known}/${expectedPod}${ALLOW_NETWORK_REPAIR?'（显式网络补核已开启）':'，缺证据保持未知且未调用CE接口'}`});
}
