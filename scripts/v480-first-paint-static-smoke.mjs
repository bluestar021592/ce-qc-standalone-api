...
assert.match(indexHtml,/v581-stable-shell-owner\.js\?v=20260925-v585-1/,'V581 stable shell must ship in the normal dashboard HTML');
...
assert.match(stableShell,/2026-09-25-v585-pointerdown-route-owner-v1/);
...
assert.match(stableShell,/doc\.addEventListener\('pointerdown',hardNavigateSidebar,true\)/,'V585 must own the earliest sidebar pointer event before legacy blockers');
assert.match(stableShell,/doc\.addEventListener\('click',hardNavigateSidebar,true\)/,'V585 must retain completed-click fallback');
assert.match(stableShell,/typeof global\.navigatePage==='function'/,'V585 must route through the app owner when available');
assert.match(stableShell,/global\.history\?\.pushState\?\./,'V585 must provide an early same-document route fallback before app bootstrap');
assert.match(stableResponse,/2026-09-25-v585-pointerdown-route-response-v1/);
...
const stableAt=indexHtml.indexOf('/v581-stable-shell-owner.js?v=20260925-v585-1');
