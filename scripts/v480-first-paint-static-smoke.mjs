...
assert.match(stableShell,/2026-09-26-v591-srcdoc-isolated-sidebar-v1/);
...
assert.match(stableShell,/srcdoc-isolated sidebar iframe/i,'V591 must explicitly isolate sidebar navigation from parent document click handlers');
...
assert.match(stableShell,/ce-qc-v591-sidebar-frame/,'V591 must mount one dedicated sidebar iframe');
assert.match(stableShell,/frame\.srcdoc=markup/,'V591 must render the navigation document from srcdoc without a second authenticated HTML request');
assert.match(stableShell,/target="_top"/,'V591 srcdoc links must navigate the top-level application');
assert.match(stableShell,/hrefFor\(path\)/,'V591 srcdoc routes must remain native hrefs');
assert.match(stableShell,/getBoundingClientRect/,'V591 must match the live parent sidebar width so no dead sliver remains');
assert.match(stableShell,/z-index:2147483647/,'V591 iframe must sit above legacy application blockers');
assert.doesNotMatch(stableShell,/sidebar-v590\.html/,'V591 primary navigation must not depend on the V590 network iframe route');
