...
// Current-day and history web routes are contract-owned. Never bind this gate to a Vxxx release label.
...
// Selected business current truth must use its own latest VALID facts + strict ledger evidence.
...
assert.match(daily, /firstReportDate/);
...
// The compact cache must persist strict first-attempt and PP/PV signing facts and stay network-free.
for (const token of [
...
// Heavy history/evidence work stays isolated in a child and derives from saved members + strict ledger facts.
...
// Generic history is cache-only on the web process.
...
// Main history UI reuses shared memory and only repeats a lightweight DOM/memory ownership guard.
...
// Visible SPA title is authoritative before URL fallback; PP/PV signing columns remain present.
assert.match(dailyUi, /document\.getElementById\('pageTitle'\)/);
for (const label of ['SHOPEE CN', 'SHOPEE VN', 'TBKH']) {
  assert.ok(dailyUi.includes(`title.includes('${label}')`), `V308 SPA title routing missing ${label}`);
}
assert.match(dailyUi, /location\.pathname/);
assert.match(dailyUi, /history=all/);
assert.match(dailyUi, /\/api\/v328\/evidence-status/);
for (const label of ['平均签收天数', '金边PP平均签收天数', '外省PV平均签收天数', '首次日报锁定日期']) {
  assert.ok(dailyUi.includes(label), `V308 history UI missing ${label}`);
}
...
// 1/2/3 attempt and first-attempt owners must use the same saved-history payload and SPA identity.
assert.match(attemptUi, /document\.getElementById\('pageTitle'\)/);
for (const label of ['SHOPEE CN', 'SHOPEE VN', 'TBKH']) {
  assert.ok(attemptUi.includes(`t.includes('${label}')`), `V328 SPA title routing missing ${label}`);
}
...
// Injection contracts check active script ownership, not cache-bust version strings.
for (const script of ['v320-history-trend-owner.js', 'v328-three-business-attempt-owner.js', 'v295-first-attempt-ui.js']) {
  assert.ok(inject.includes(script), `UI injection missing ${script}`);
}
...
console.log('[V321] web availability behavior gate passed · per-business current truth · strict ledger · PP/PV signing · shared cache-only history · SPA title ownership · no release-name coupling');
