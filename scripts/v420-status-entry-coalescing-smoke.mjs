...
assert.match(v169,/name==='runUnified'&&exactShopeeRestartInterruption\(state\)&&typeof originalEntries\.resumeUnified==='function'[\s\S]*return originalEntries\.resumeUnified\.apply\(this,arguments\)/,'explicit Start must delegate exact restart proof to the original V67 resume entry');
...
assert.match(shell,/<script src=\"\/v169-seven-business-legacy-status-sync\.js\?v=20260904-v423-1\"/,'browser shell must actually request the V423 entry bridge with a new cache-bust URL');
...
assert.doesNotMatch(shell,/<script src=\"\/v169-seven-business-legacy-status-sync\.js\?v=20260904-v421-1\"/,'retired V421 live URL must not keep browser execution on the pre-restart-routing build');
...
console.log('[V423/V422/V421/V420] unified-entry delivery smoke passed · browser shell cache-busts V423/V420 current builds · V168 display stays fail-closed · Start remains clickable through bounded confirmation · exact current-date Shopee restart after CCSL completion routes only to original V67 resume · generic failures remain normal-start · V412 no duplicate preflight · V67 sole runner');