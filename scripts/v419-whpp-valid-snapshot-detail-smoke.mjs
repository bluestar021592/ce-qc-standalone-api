...
try{
  const good='WH-V419-VALID',bad='WH-V419-INVALID',invalidOnly='WH-V419-INVALID-ONLY';
  ...
  assert.deepEqual(range.dates,['2026-08-01'],'an INVALID-only snapshot date must not create a historical membership date');
  assert.equal(range.total,1);
  assert.equal(range.rows[0]?.shipmentCode,good);
  assert.ok(!range.rows.some(row=>row.shipmentCode===bad||row.shipmentCode===invalidOnly),'invalid snapshot members must never leak into WHPP range detail');
  console.log('[V419 WHPP VALID SNAPSHOT DETAIL] PASS invalid/failed snapshots cannot override completed history or create phantom range dates; latest current POD still overlays valid daily membership');
}finally{
  try{closeDb();}catch{}
