...
function scan(orderStatus, extra = {}) {
  return { shipmentCode: bill, orderStatus, ...extra };
}

function event(code, eventTime, extra = {}) {
  return { shipmentCode: bill, eventCode: String(code), eventTime, ...extra };
}

function analyze({ orderStatus = '70', events = [], exceptions = [], analysisDate = '2026-08-09' } = {}) {
  return analyzeShopeeShipment({
    waybill: bill,
    scanRow: scan(orderStatus),
    shipmentTrackRow: {},
    events,
    exceptions,
...
test('tracking 30 or 32 is cycle count and closes prior 26 state', () => {
  for (const code of ['30', '32']) {
    const result = analyze({ events: [
      event('26', '2026-08-08T08:00:00+07:00'),
      event(code, '2026-08-09T08:00:00+07:00')
    ] });
...
test('same-day repeated 150 counts as one Pending natural day', () => {
  const result = analyze({ events: [
    event('150', '2026-08-09T08:00:00+07:00'),
    event('150', '2026-08-09T12:00:00+07:00'),
    event('150', '2026-08-09T18:00:00+07:00')
  ] });
...
test('150 on three distinct dates counts 3 Pending days and detects discontinuity', () => {
  const result = analyze({ events: [
    event('150', '2026-08-05T08:00:00+07:00'),
    event('150', '2026-08-06T08:00:00+07:00'),
    event('150', '2026-08-08T08:00:00+07:00')
  ] });
...
test('historical Pending is cleared by a newer effective node', () => {
  const result = analyze({ events: [
    event('150', '2026-08-07T08:00:00+07:00'),
    event('150', '2026-08-08T08:00:00+07:00'),
    event('30', '2026-08-09T08:00:00+07:00')
  ] });
  assert.equal(result.currentState, 'CYCLE_COUNT');
  assert.equal(result.Pending当前次数, 0);
...
test('tracking 99 latest is work-order current state', () => {
  const result = analyze({ events: [
    event('26', '2026-08-08T08:00:00+07:00'),
    event('99', '2026-08-09T08:00:00+07:00')
  ] });
...
test('tracking 80 closes POD regardless of historical anomalies', () => {
  const result = analyze({ events: [
    event('150', '2026-08-07T08:00:00+07:00'),
    event('30', '2026-08-08T08:00:00+07:00'),
    event('80', '2026-08-09T08:00:00+07:00')
  ] });
...
test('tracking 84 is return in progress; 86 is normal completed return', () => {
  const inProgress = analyze({ events: [
    event('150', '2026-08-07T08:00:00+07:00'),
    event('84', '2026-08-09T08:00:00+07:00')
  ] });
  ...
  const completed = analyze({ events: [
    event('84', '2026-08-08T08:00:00+07:00'),
    event('86', '2026-08-09T08:00:00+07:00')
  ] });
