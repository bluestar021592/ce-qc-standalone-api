...
test('V505 finalized receipt is terminal across admins and purge generations without deleting newer PREPARE evidence',async()=>{
  ...
  try{
    ...
    fs.writeFileSync(prepareFile,JSON.stringify({
      ...
      payload:{challengeId:newChallenge,expiresAt:futureIso}
    },null,2),'utf8');
    fs.writeFileSync(challengeFile,JSON.stringify({
      ...
      challenge:{
        ...
        backup:{filePath:fakeBackup,sha256:'b'.repeat(64),size:123,mtimeMs:1,integrity:'ok',method:'test'},
        sourceFingerprint:{db:{exists:true,size:1,mtimeMs:1},wal:{exists:true,size:1,mtimeMs:1}}
      },
      payload:{challengeId:newChallenge,expiresAt:futureIso}
    },null,2),'utf8');
