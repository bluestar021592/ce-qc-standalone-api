...
test('V505 detached execute returns quickly, durably binds the sealed DB path, reuses one live job, and finishes transactional purge out of the HTTP process',async()=>{
  ...
  try{
    ...
    const completedSidecar=JSON.parse(fs.readFileSync(executeJobFile,'utf8'));
    assert.equal(completedSidecar.status,'SUCCEEDED');assert.equal(completedSidecar.jobId,first.jobId);assert.equal(completedSidecar.result?.countSource,'DELETE_CHANGESET_EXACT');assert.equal(completedSidecar.result?.backupSourceFingerprint,'MATCHED_UNDER_BEGIN_IMMEDIATE');assert.ok(['ok','committed-with-warnings'].includes(completedSidecar.result?.integrity));assert.equal(completedSidecar.result?.committedReceipt,true);assert.equal(completedSidecar.result?.executeJobId,first.jobId);
    const recovered=inspectExecutionRecovery(user);
    if(recovered){assert.equal(recovered.status,'SUCCEEDED');assert.equal(recovered.jobId,first.jobId);assert.equal(recovered.completed,true);}else{assert.equal(fs.existsSync(executeJobFile),false,'after public SUCCEEDED, a finalized exact sidecar may be retired instead of replaying a second terminal payload');}
    const receipt=readPurgeCommitReceipt({challengeId:challenge.challengeId,executeJobId:first.jobId});assert.ok(receipt,'normal execute must leave an exact durable commit receipt');assert.equal(receipt.executeJobId,first.jobId);assert.equal(receipt.finalizationState,'SAFE_POSTCHECK_PASSED');assert.ok(receipt.finalizedAt,'normal execute must durably record finalization before terminal cleanup');
