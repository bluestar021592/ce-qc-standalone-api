...
test('general track 30/32 latest means cycle count and clears prior 26', () => {
  for (const code of ['30', '32']) {
    const row = analyze([
      event('26', '2026-08-08 08:00:00'),
      event(code, '2026-08-09 08:00:00')
    ]);
...
test('general same-day repeated 150 is exactly one Pending natural day', () => {
  const row = analyze([
    event('150', '2026-08-09 08:00:00'),
    event('150', '2026-08-09 12:00:00'),
    event('150', '2026-08-09 18:00:00')
  ]);
...
test('general Pending count uses actual 150 dates, not elapsed days', () => {
  const row = analyzeShipment({
    waybill: bill,
    scanRow,
    events: [event('150', '2026-08-05 08:00:00')],
...
test('general newer 99 work order clears historical Pending current-state anomaly', () => {
  const row = analyze([
    event('150', '2026-08-07 08:00:00'),
    event('150', '2026-08-08 08:00:00'),
    event('99', '2026-08-09 08:00:00')
  ]);
...
test('general track 80 is POD terminal and clears ordinary anomaly counters', () => {
  const row = analyze([
    event('150', '2026-08-07 08:00:00'),
    event('30', '2026-08-08 08:00:00'),
    event('80', '2026-08-09 08:00:00')
  ]);
...
test('general 84 is return-in-progress and 86 is normal completed return', () => {
  ...
  const completed = analyze([
    event('84', '2026-08-08 08:00:00'),
    event('86', '2026-08-09 08:00:00')
  ]);
...
test('general broad signed text without code 80 does not fabricate POD terminal', () => {
  const row = analyze([
    event('', '2026-08-09 08:00:00', { trackingEventDescZh: '已签收测试文字但没有状态码80' })
  ]);
