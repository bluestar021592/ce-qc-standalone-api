...
const runner = fs.readFileSync(new URL('../public/whpp-v47-auto-run.js', import.meta.url), 'utf8');

test('V54 is now a lightweight compatibility layer and never replaces the resilient seven-business runner', () => {
  assert.match(integration, /V47 is now the authoritative seven-business start\/resume orchestrator/);
...
test('V54 compatibility runtime is injected after V47 with a fresh cache key', () => {
  const v47 = injector.indexOf('/whpp-v47-auto-run.js?v=20260812-3');
  const v54 = injector.indexOf('/v54-whpp-unified-integration.js?v=20260812-8');
  assert.ok(v47 >= 0 && v54 >= 0, 'V47 and V54 runtimes must be present');
  assert.ok(v54 > v47, 'V54 pause bridge may load after V47 because it does not replace run/resume');
});
