...
test('V142 history audit is read-only and covers seven businesses, carry and evidence tables',()=>{
  ...
  assert.match(s,/exportReady/);assert.match(s,/BLOCKED_UNTIL_REPAIRED/);assert.match(s,/CEAF_SOURCE_MEMBERSHIP_MISMATCH/);
  assert.doesNotMatch(s,/\b(?:INSERT|UPDATE|DELETE|REPLACE)\s+(?:INTO|FROM|OR)?/i);
});

test('V142 strict exporter includes CEAF and WHPP and cannot silently skip a requested date',()=>{
...
test('V142 replaces both period export endpoints and exposes read-only audit endpoint',()=>{
  syntax('src/v142SevenBusinessExportPatch.js');const s=read('src/v142SevenBusinessExportPatch.js');
  assert.match(s,/\/api\/export-period/);assert.match(s,/\/api\/export-period\/prepare/);assert.match(s,/\/api\/v142\/history-integrity/);assert.match(s,/V142-SEVEN-BUSINESS-STRICT/);
});
...
test('V141 and V142 are wired into bootstrap and import-page UI',()=>{
  ...
  assert.match(boot,/v141WhppDailyRetryIsolationPatch/);assert.match(boot,/v142SevenBusinessExportPatch/);
  assert.match(ui,/v141-whpp-retry-isolation-ui\.js/);assert.match(ui,/v142-history-integrity-audit\.js/);
});

test('legacy five-business period exporter is no longer the registered route authority',()=>{
