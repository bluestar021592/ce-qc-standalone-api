...
test('V94 runtime is syntax-valid and loaded after V90 before the actual main server start', () => {
  ...
  assert.ok(serverStart>=0&&bootstrap.indexOf('v94ShopeeWhppSourceTruthPatch') < serverStart);
  assert.match(read('src/shopeeAnalyzer.js'), /shopeeAnalyzerV33\.js/);
});

test('WHPP retention accepts only a latest event that still locates the parcel at WHPP', () => {
  assert.equal(isStrictShopeeWhppRetention({ event: { eventCode: '50', eventTime: '2026-08-01 10:00:00', trackingEventDescZh: '货物到达网点【CE:WHPP】' }, scanOrderStatus: '50' }), true);
  assert.equal(isStrictShopeeWhppRetention({ event: { eventCode: '26', eventTime: '2026-08-01 10:00:00', locationCode: 'CE:WHPP' }, scanOrderStatus: '60' }), true);
});

test('POD, completed return and return-in-progress can never enter WHPP retention', () => {
  for (const eventCode of ['80', '84', '86']) {
    assert.equal(isStrictShopeeWhppRetention({ event: { eventCode, locationCode: 'CE:WHPP', trackingEventDescZh: 'CE:WHPP' }, scanOrderStatus: '50' }), false, `eventCode=${eventCode}`);
  }
  for (const orderStatus of ['85', '100']) {
    assert.equal(isStrictShopeeWhppRetention({ event: { eventCode: '26', locationCode: 'CE:WHPP' }, scanOrderStatus: orderStatus }), false, `orderStatus=${orderStatus}`);
  }
  assert.equal(isStrictShopeeWhppRetention({ event: { eventCode: '26', locationCode: 'CE:WHPP' }, scanOrderStatus: '50', finalRow: { currentState: 'RETURN_COMPLETED', primaryCategory: '退回', 退回状态: '已退回' } }), false);
});

test('an outbound or return sentence that merely mentions WHPP is not WHPP retention', () => {
  assert.equal(isStrictShopeeWhppRetention({ event: { eventCode: '26', eventTime: '2026-08-01 10:00:00', trackingEventDescZh: '货物离开网点【CE:WHPP】，下一个网点【CE:CP123456】' }, scanOrderStatus: '60' }), false);
  assert.equal(isStrictShopeeWhppRetention({ event: { eventCode: '26', eventTime: '2026-08-01 10:00:00', trackingEventDescZh: '包裹从 CE:WHPP 退回处理中' }, scanOrderStatus: '60' }), false);
  assert.equal(isStrictShopeeWhppRetention({ event: { eventCode: '26', eventTime: '2026-08-01 10:00:00', trackingEventDescZh: '操作备注提到 CE:WHPP，但当前未提供WHPP到达/当前位置证据' }, scanOrderStatus: '60' }), false);
});
