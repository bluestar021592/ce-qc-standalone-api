...
function stateFixture() {
  ...
  const dailyParseRows = [
    ['WHPP-PP-SHOP','PP'],['WHPP-PV-SHOP','PV'],['WHPP-PENDING','PP'],['WHPP-POD','PV'],['WHPP-580','PV']
  ].map(([shipmentCode, regionCode]) => ({ shipmentCode, regionCode, reportDate, businessType:'WHPP' }));
  const finalRows = [
    {shipmentCode:'WHPP-PP-SHOP',reportDate,regionCode:'PP',businessType:'WHPP',currentState:'PICKUP_SUCCESS',primaryCategory:'正常流转',shopState:'SHOP_ARRIVED_CURRENT',Pending次数:3,Pending当前次数:3,OC天数:0},
    {shipmentCode:'WHPP-PV-SHOP',reportDate,regionCode:'PV',businessType:'WHPP',currentState:'PICKUP_SUCCESS',primaryCategory:'正常流转',shopState:'SHOP_TRANSFER_IN_PROGRESS',Pending次数:0,OC天数:3},
    {shipmentCode:'WHPP-PENDING',reportDate,regionCode:'PP',businessType:'WHPP',currentState:'PENDING',primaryCategory:'Pending3次以上',Pending次数:3,Pending当前次数:3},
    {shipmentCode:'WHPP-POD',reportDate,regionCode:'PV',businessType:'WHPP',currentState:'POD',primaryCategory:'POD',是否POD:'是',POD状态:'POD'},
    {shipmentCode:'WHPP-580',reportDate,regionCode:'PV',businessType:'WHPP',currentState:'CCSL580_RETENTION',primaryCategory:'CCSL580_RETENTION',specialState:'CCSL580_RETENTION'}
  ];
  return {businessType:'WHPP',reportDate,pnhBills:dailyParseRows.map(row=>row.shipmentCode),dailyParseRows,finalRows};
}
...
test('WHPP UI exposes both shop cards and uses matching detail tabs', () => {
  ...
  assert.match(injector, /whpp-v44\.js\?v=20260812-2/);
});
