...
test('automated track/event/exception APIs receive only bills whose scan status is 50 60 or 70', async () => {
  ...
  client.postJson = async (path, body) => {
    ...
    if (path.includes('tms-shipment-event/query')) {
      return { data: body.map(code => ({ shipmentCode: code, eventCode: '26', eventTime: '2026-08-13 10:00:00' })) };
    }
  ...
  const all = ['A50','B60','C70','D85','E100','F99'];
  ...
  assert.ok(!events.some(row => ['D85','E100'].includes(row.shipmentCode)), 'POD/return terminals remain scan-only closures');
});
...
test('Shopee analyzer preserves unknown scan status as local hold while scan 85/100 remain terminal', () => {
