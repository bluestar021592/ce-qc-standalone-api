...
test('V13 unified import accepts 收件人名称 after title rows, short filename date, seven-business priority and province mapping', () => {
  const dir = tempDir();
  ...
  rows.push(['CC00000000001', 'NORMAL CUSTOMER', '', '2026-08-03 11:00:00']);
  ...
  assert.deepEqual(result.classificationCounts, { CE: 1, CEAF: 0, TBKH: 1, ALI1688: 1, SHOPEECN: 1, SHOPEEVN: 2, WHPP: 0 });
  ...
  assert.deepEqual(result.sourceReconciliation.businessTypes, ['CE','CEAF','TBKH','ALI1688','SHOPEECN','SHOPEEVN','WHPP']);
  ...
  assert.equal(result.rows.find(row => row.shipmentCode === 'CC00000000001')?.regionCode, '');
...
test('V13 explicit 日报日期 outranks filename and transaction dates', () => {
  ...
  writeBook(file, [
    ...
    ['CC00000000002', 'NORMAL', 'Phnom Penh', '2026-08-05', '2026-08-03 10:00:00'],
    ['CC00000000003', 'NORMAL', 'Kandal', '2026-08-05', '2026-08-03 10:00:00']
  ]);
...
test('V13 recipient value heuristic supports unfamiliar recipient header when tagged business values exist', () => {
  ...
  assert.equal(result.classificationCounts.WHPP, 0);
  assert.equal(result.sheetDiagnostics[0]?.detectedColumns?.recipientDetection, 'VALUE_HEURISTIC');
});

test('V13 duplicate waybill with conflicting business evidence fails closed instead of keeping the first classification', () => {
  const dir = tempDir();
  const file = path.join(dir, '2026-08-07.xlsx');
  writeBook(file, [
    ['运单号', '收件人名称', '省份'],
  ...
  assert.throws(
    () => parseUnifiedDailyExcel(file, { originalName: path.basename(file) }),
    error => {
      assert.equal(error?.code, 'DUPLICATE_BUSINESS_CLASSIFICATION_CONFLICT');
      assert.equal(error?.shipmentCode, 'DUP000000001');
      assert.equal(error?.first?.businessType, 'SHOPEECN');
      assert.equal(error?.duplicate?.businessType, 'SHOPEEVN');
      return true;
    }
  );
});