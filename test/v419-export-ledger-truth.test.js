...
test('WHPP export takes POD, strict attempt and START-to-POD days from V246 ledger after stale legacy evidence',()=>{
  const db=fixture();try{
    ...
    const rows=[{shipmentCode:'WHPP-EXPORT-1',businessType:'WHPP',pod:false,returned:true,pending:true,delivering:true,statusCode:'R',statusDesc:'RETURNED',attemptNo:3,podAttemptNo:3,currentAttemptNo:3,signingDays:9,deliveryDays:9,evidence:new Set()}];
    ...
    assert.equal(row.statusCode,'Y');assert.equal(row.statusDesc,'POD');assert.equal(row.podDate,'2026-08-03');
    ...
    // WHPP is not a strict delivery KPI type, so its existing V329 first-report signing day is preserved unless ledger signingDays exists.
    assert.equal(row.signingDays,9);
    assert.equal(row.v419CanonicalExportTruthId,'2026-09-03-v419-canonical-export-ledger-truth-v1');
  }finally{db.close();}
});
...
test('OPEN ledger clears stale historical POD and RETURNED ledger clears stale POD',()=>{
  const db=fixture();try{
    ...
    const open={shipmentCode:'TBKH-OPEN',pod:true,returned:false,pending:false,delivering:false,statusCode:'Y',statusDesc:'POD',attemptNo:1};
    const returned={shipmentCode:'CE-RETURNED',pod:true,returned:false,pending:true,delivering:true,statusCode:'Y',statusDesc:'POD',attemptNo:1};
    applyV419CanonicalExportLedgerTruth('TBKH',[open],{db});
    applyV419CanonicalExportLedgerTruth('CE',[returned],{db});
    assert.equal(open.pod,false);assert.equal(open.returned,false);assert.equal(open.attemptNo,0);assert.equal(open.statusDesc,'OC');
    assert.equal(returned.pod,false);assert.equal(returned.returned,true);assert.equal(returned.pending,false);assert.equal(returned.delivering,false);assert.equal(returned.statusCode,'R');assert.equal(returned.statusDesc,'RETURNED');
  }finally{db.close();}
});
