...
test('outbound to exact whitelist store is in transit', () => {
  const result = analyzeStoreFlow({ shipmentCode: 'T1', reportDate: '2026-08-04', events: [
    event('2026-08-03 10:00:00', '货物离开网点【CEL:CEZT】，下一个网点为【CEL:CP000457】')
  ] });
  assert.equal(result.shopState, 'SHOP_TRANSFER_IN_PROGRESS');
  assert.equal(result.targetShopCode, 'CP000457');
});
...
test('arrival changes transit to current store and calculates natural days', () => {
  const result = analyzeStoreFlow({ shipmentCode: 'T2', reportDate: '2026-08-04', events: [
    event('2026-08-02 10:00:00', '货物离开网点【CEL:CEZT】，下一个网点为【CEL:CP000457】'),
    event('2026-08-03 11:00:00', '货物到达网点【CEL:CP000457】')
  ] });
  assert.equal(result.shopState, 'SHOP_ARRIVED_CURRENT');
  assert.equal(result.currentShopCode, 'CP000457');
...
test('pending at current store is an auxiliary state', () => {
  const result = analyzeStoreFlow({ shipmentCode: 'T3', reportDate: '2026-08-04', events: [
    event('2026-08-03 10:00:00', 'Outbound next node CEL:CP000457'),
    event('2026-08-03 11:00:00', 'Inbound CEL:CP000457'),
    event('2026-08-04 12:00:00', 'Pending 无法联系客户', 'CP000457')
  ] });
  assert.equal(result.shopState, 'SHOP_ARRIVED_CURRENT');
...
test('POD and return close a store cycle', () => {
  const events = [event('2026-08-03 10:00:00', 'Outbound next node CEL:CP000457')];
